...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`tools/mcp.md`。
...
## MCP 协议详解
  ...
  Anthropic 2024 年 11 月推出的开放协议——把"每家 LLM 应用都重新发明一遍工具集成"这件事，变成一次写完到处接的标准。它和 function calling 是两层东西，混淆这个是面试高频翻车点。

  
    适合阶段：Agent 进阶 / 生产
    核心链路：Host ↔ Client ↔ Server
    面试重点：协议分层 + 与 function calling 的边界
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    MCP 和 OpenAI function calling 是什么关系？替代还是补充？
    考分层意识——能不能区分"LLM API 协议"和"工具进程协议"，这是 MCP 类问题的 0 分门槛。
  

  
    MCP 的三种 primitive（Tool / Resource / Prompt）各自做什么？什么时候选哪个？
    考是否真读过 spec——很多人把 Resource 当 Tool 写，本质就没分清。
  

  
    Host、Client、Server 三者各自的职责？为什么是 1 个 Host 对 N 个 Server？
    考架构理解。能不能讲清"每个 Server 都有自己专属的 Client 实例"。
  

  
    stdio 和 Streamable HTTP 两种 transport 各自适合什么场景？
    考工程选型——本地工具 vs 远程服务。SSE 已经被废弃这个点能答出来加分。
  

  
    为什么 MCP 用 JSON-RPC 而不是 REST？
    考协议设计常识。双向、有状态、低延迟——这是 LSP 路线选 JSON-RPC 的原因。
  

  
    MCP Server 跑在用户本地，安全边界怎么设？
    考生产意识。tool descriptions 是不可信输入、需要用户授权、间接 prompt injection。
  

  
    capability negotiation 是什么？为什么需要它？
    考对 stateful 协议的理解，能联系到 LSP 同源设计。
  

  
    MCP 会不会被 OpenAI / Google 的私有协议取代？
    考行业判断。OpenAI 2025 年宣布支持 MCP 是这道题的关键事实。
