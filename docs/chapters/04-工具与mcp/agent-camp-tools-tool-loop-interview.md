# 工具调用死循环面试深挖：别只答 max_steps

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`tools/tool-loop-interview.md`。
...
## 工具调用死循环面试深挖：别只答 max_steps
  ...
  Agent 一直调工具，不是模型“笨”，通常是你把太多决策权塞进了同一个 loop。

  
    适合阶段：Agent 工程师 / 平台面
    核心能力：Tool Routing · State Machine · Loop Guard · Verifier
    脱敏原则：只保留工程方法，不保留项目细节
...
这组问题考的不是你会不会调 `tools=[...]`，而是你有没有把 Agent 当成一个会失控的生产系统来设计。
    ...
    你遇到过 Agent 工具调用死循环吗？根因是什么？
    考你是否只会怪模型，还是能讲出工具选择空间、状态和终止条件。
  

  
    为什么加 max_steps 不能算真正修复？
    考你能不能区分“保险丝”和“根因修复”。
  

  
    工具很多时，为什么不能一次性全塞给模型？
    考 context engineering：工具召回、按意图加载、namespace 和候选集裁剪。
  

  
    哪些逻辑应该让模型决定，哪些应该下沉到代码？
    考确定性边界：日期、卡片、枚举、权限、业务状态不该靠模型猜。
  

  
    怎么检测“同一个工具同一组参数反复调用”？
    考工程实现：tool_call fingerprint、状态更新、no-progress detector。
  

  
    工具调用 loop 和 LangGraph 的递归限制是什么关系？
    考框架知识：recursion limit 是最后防线，条件边和 END 才是正常终止机制。
  

  
    如果工具返回 200 但业务失败，为什么也会导致 loop？
    考 tool_result 设计：业务错误要结构化暴露，而不是伪装成成功数据。
  

  
    如果现在重做这套工具调用架构，你会怎么设计？
    考平台抽象：router、registry、executor、guard、trace、eval 能不能串成闭环。
...
class ToolRuntime:
    ...
    def run(self, user_text: str, policy: Callable[[str, LoopState], ToolCall]) -> str:
        ...
        for step in range(1, self.guard.max_steps + 2):
            ...
            result = self.tools[call.name](https://github.com/yibo365/agent-camp)
            if not result.ok:
