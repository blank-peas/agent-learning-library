...
> **资料来源**：[Agent Interview 100](https://github.com/BigKunLun/Agent-Interview-100) · 原文件：`03-tool-use/021-function-calling-basics.md`。
...
sequenceDiagram
    ...
    U->>L: 提问："北京天气？"
    L-->>A: tool_use(JSON)：get_weather(city="Beijing")
    A->>A: 本地执行工具（LLM 不执行）
    A->>L: tool_result：{temp: 22}
    L-->>U: 最终回答："北京 22°C"
```
*LLM 只生成调用请求、不执行工具：请求与结果在用户、LLM、应用代码之间往返一圈才得到回答。*
