...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`tools/parallel.md`。
...
## 并行工具调用
  ...
  用户问"对比 A 和 B"，模型本可以一次发起两个 tool_calls 同时跑——串行就是把延迟翻倍交给用户。

  
    适合阶段：进阶 / 生产
    核心：一次响应多个 tool_calls + 并发执行
    面试重点：ID 配对、依赖判断、部分失败处理
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    模型怎么决定要不要并行调用？是规则还是训练出来的？
    考你能不能讲清"并行决策"不是 prompt 能强控的。
  

  
    OpenAI / Anthropic / Google 三家的并行 function calling 协议有什么差别？
    考三家 API 细节，能不能讲出 tool_calls 数组 vs content block 的区别。
  

  
    并行 tool_calls 的 ID 配对是怎么实现的？返回顺序错了会怎样？
    考协议本质，ID 是配对锚点不是顺序锚点。
  

  
    并行调用中一个工具失败了怎么办？要全部回滚吗？
    考部分失败的工程处理，不是非黑即白的答案。
  

  
    怎么强制模型不要并行？怎么强制并行？
    考 parallel_tool_calls 参数的存在与边界。
  

  
    asyncio.gather 一个 task 抛异常会怎样？怎么避免拖垮整个 batch？
    考 Python 并发的细节，return_exceptions 的用法。
  

  
    模型把"先建文件夹再写文件"这种有依赖的操作并行调了，怎么办？
    考反模式识别，依赖判断的责任在哪里。
