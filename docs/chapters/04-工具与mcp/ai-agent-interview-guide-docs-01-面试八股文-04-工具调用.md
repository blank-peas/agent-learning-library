...
> **资料来源**：[AI Agent Interview Guide](https://github.com/bcefghj/ai-agent-interview-guide) · 原文件：`docs/01-面试八股文/04-工具调用.md`。
...
def eval_expr(node: ast.AST) -> float:
    ...
    if isinstance(node, ast.BinOp) and type(node.op) in _ALLOWED:
        return _ALLOWED[type(node.op)](https://github.com/bcefghj/ai-agent-interview-guide), eval_expr(node.right))
    if isinstance(node, ast.UnaryOp) and type(node.op) in _ALLOWED:
        return _ALLOWED[type(node.op)](https://github.com/bcefghj/ai-agent-interview-guide))
