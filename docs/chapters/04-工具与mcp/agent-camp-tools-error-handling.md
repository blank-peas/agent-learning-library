...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`tools/error-handling.md`。
...
## 工具调用的错误处理与重试
  ...
  工具会失败，但 Agent 不能跟着崩——失败模式分类、重试策略、降级、给模型看的错误信息怎么写。
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    给模型看的 error message 和给开发者看的 log 区别在哪？为什么不能复用？
    考受众意识——模型需要"怎么修"，开发者需要"完整堆栈"，混用会双输。
  

  
    tool 调用失败该不该重试？你按什么判断？
    考能不能按错误类别决策——区分瞬时错误、确定性错误、需要人工介入的错误。
  

  
    怎么防止模型反复调失败的工具陷入死循环？
    考工程化思维——硬上限、循环检测、参数相同检测三层防护。
  

  
    401 / 429 / 500 各自该怎么处理？
    考具体 HTTP 错误码到 Agent 行为的映射——不是所有 5xx 都该重试。
  

  
    让框架强制重试 vs 让模型自己决定重试，各有什么适用场景？
    考对"控制权"的理解——延迟敏感时框架重试，需要语义判断时让模型决定。
  

  
    Circuit breaker 在 LLM Agent 里怎么用？
    考分布式系统经验能不能迁移过来——保护后端服务 + 给模型清晰的"该换路了"信号。
  

  
    tool 失败时返回 fallback 值还是直接告诉模型不可用，怎么选？
    考降级策略的辨析——静默 fallback 容易让模型基于错数据继续推理。
  

  
    上游 tool 的错误结果会污染下游 Agent 的判断吗？怎么隔离？
    考多 Agent 错误传染问题，结合上下文污染的概念。
