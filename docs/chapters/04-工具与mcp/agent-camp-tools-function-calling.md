# Function Calling 函数调用规范

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`tools/function-calling.md`。
...
## Function Calling 函数调用规范
  ...
  让模型不再"假装"调用工具——它真正吐出一段结构化 JSON，你的业务代码拿到 JSON 执行，结果再喂回去。这是 ReAct 文本范式的工程化版本。

  
    适合阶段：Agent 入门 / 生产
    核心链路：模型 → tool_call → 业务执行 → tool_result → 继续生成
    面试重点：三家协议差异 + 与 ReAct 的关系
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    function calling 和 ReAct prompt 是什么关系？
    考你能不能把"文本范式"和"工程化协议"讲清——不要回答成两件无关的事。
  

  
    OpenAI / Anthropic / Google 三家 tool_call 的消息格式差异在哪？
    考真实用过几家 SDK——能讲出 tool_calls 数组 vs content block、role 字段差异等细节。
  

  
    tool_choice 的 auto / required / 指定工具名 / none 分别是什么语义？
    考对 API 细节的掌握，是不是只用过默认值。
  

  
    为什么对齐良好的模型 function calling 仍然可能调错参数？
    考底层原理——模型生成 JSON 的本质仍是 token 预测，不是结构化约束。
  

  
    并行返回多个 tool_calls，怎么把多个结果同时回传？顺序重要吗？
    考工程细节，tool_call_id 配对、消息顺序约束。
  

  
    tool_result 写得不好会怎样影响下一轮？
    考工程经验——返回噪声、过长、结构不一致的实际后果。
  

  
    function calling 底层依赖什么标准？为什么是 JSON Schema 而不是别的？
    考底层认知——JSON Schema Draft 7+、模型对 schema 的"训练"程度。
  

  
    什么场景下应该退回 ReAct 文本 prompt，不用原生 function calling？
    考辨析能力——模型支持度、可解释性、可控性的权衡。
