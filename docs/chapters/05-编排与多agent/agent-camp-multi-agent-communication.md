# Agent 通信协议 A2A / ACP / AGNTCY

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`multi-agent/communication.md`。
...
## Agent 通信协议 A2A / ACP / AGNTCY
  ...
  MCP 解决了"Agent 怎么调工具"，没解决"Agent 怎么调 Agent"。2025 年三家分别推出了 A2A、ACP、AGNTCY 来填这个空——半年后 ACP 已经并入 A2A，AGNTCY 落进 Linux Foundation。这篇讲清楚它们的分工、消息模型和真实的标准之争。

  
    适合阶段：Agent 进阶 / 生产
    核心链路：Agent ↔ Agent（水平）vs Agent ↔ Tool（垂直）
    面试重点：协议分层 + 与 MCP 的边界
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    MCP 和 A2A 都是 2024-2025 推的协议，区别在哪？两者冲突还是互补？
    考分层意识——能不能区分"Agent 调工具"和"Agent 调 Agent"。这是这道题的 0 分门槛。
  

  
    A2A 的 AgentCard 设计解决了什么问题？为什么不直接用 OpenAPI？
    考对"Agent 发现 + 能力公告"的理解，能联系到 robots.txt / well-known URL 模式更佳。
  

  
    ACP 和 A2A 都是 Agent 间通信协议，本质冲突吗？为什么 ACP 后来并入 A2A？
    考对协议演进的认知，知道 2025-08 IBM ACP 已经停止独立发展并入 A2A。
  

  
    AGNTCY 到底是协议还是别的？它和 A2A 有什么关系？
    考是否真看过它的定位——AGNTCY 是"Internet of Agents"全栈倡议，不只是通信协议。
  

  
    A2A 用 JSON-RPC over HTTP/SSE，为什么不用 gRPC 或 WebSocket？
    考协议设计权衡。能扯到企业代理、防火墙、CDN 友好性最好。
  

  
    现在生产里有人真用 A2A 吗？还是大家继续用自家协议？
    考行业判断。生产现状是"实验居多、内部协议为主"——能讲清现状才是真懂。
  

  
    Agent-to-Agent 通信的安全风险有哪些？比单 Agent 多了什么？
    考对 Agent-in-the-Middle、AgentCard 仿冒、跨 Agent 信任链的理解。
  

  
    一个 multi-agent 产品要不要现在就上 A2A？怎么决策？
    考工程判断——协议没成熟前过早绑定的代价。
