...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`workflow/patterns.md`。
...
## 编排模式（顺序 / 并行 / 条件 / 循环）
  ...
  所有 LLM Workflow 和 Agent 系统的控制流都是四种基础模式的组合：顺序执行、并行扇出、条件分支、循环反馈。这篇讲清楚每种模式解决什么问题、什么场景用哪种、以及 Human-in-the-Loop 怎么嵌入。
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    四种编排模式分别解决什么问题？
    考基础功——顺序解决步骤依赖、并行解决延迟、条件解决分支、循环解决迭代优化。
  

  
    什么时候该用并行，什么时候必须串行？判断标准是什么？
    考依赖关系分析——有数据依赖必须串行，无依赖才能并行。
  

  
    循环模式怎么防止死循环？
    考工程兜底——max_iterations、退出条件、超时、递减温度。
  

  
    Human-in-the-Loop 该插在哪？怎么实现"暂停-恢复"？
    考对 checkpoint + interrupt 机制的理解——不是所有位置都适合插人工审核。
  

  
    Anthropic 的五种 Workflow 模式分别对应哪些基础编排模式的组合？
    考能不能把高层模式拆解为底层原语——prompt chaining = 顺序，routing = 条件，等等。
  

  
    一个复杂的 RAG + 生成 + 审核的管线，怎么用这四种模式组合？
    考实战设计能力——不是背概念，而是能画出 graph。
