# 先规划后执行 Plan-and-Execute / ReWOO

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`agent/plan-execute.md`。
...
## 先规划后执行 Plan-and-Execute / ReWOO
  ...
  长任务不要每一步都临场发挥，先拆计划，再按步骤执行、校验和必要时重规划。

  
    适合阶段：长任务 Agent
    核心机制：Planner / Executor / Replanner
    面试重点：计划过时与依赖管理
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    Plan-and-Execute 和 ReAct 的差异是什么？
    考逐步决策和先规划再执行的取舍。
  

  
    Planner 的输出应该是自然语言清单，还是结构化任务图？
    考依赖、状态、验收标准和可执行性。
  

  
    计划执行到一半发现前提错了怎么办？
    考 replanning、partial rollback、human confirmation。
  

  
    ReWOO 为什么要把 reasoning 和 observation 解耦？
    考减少重复模型调用和变量引用。
    ...
    长任务 Agent 怎么评估？
    考 plan quality、step success、dependency violation、replan rate。
  

  
    什么时候不该用 Plan-and-Execute？
    考短任务、强交互任务、环境变化快的任务。
