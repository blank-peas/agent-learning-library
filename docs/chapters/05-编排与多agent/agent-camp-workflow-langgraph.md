...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`workflow/langgraph.md`。
...
## LangGraph 深度解析
  ...
  LangGraph 是 LangChain 团队推出的 Agent 编排框架，用有向图（节点 + 边）建模 LLM 应用的控制流。它的定位是"低层编排运行时"——不藏魔法，你完全控制每一步怎么走。Klarna、Lyft、Coinbase、LinkedIn 等 25+ 家企业在用。
    ...
    适合阶段：实战 / 生产
    核心：StateGraph + Checkpointer + Human-in-the-Loop
    面试重点：为什么选 LangGraph + 和其他框架对比
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    LangGraph 和 LangChain 是什么关系？为什么不直接用 LangChain？
    考你能不能讲清"LangChain 是工具层，LangGraph 是编排层"的定位差异。
  

  
    LangGraph 的 StateGraph 里，State、Node、Edge 分别是什么？
    考核心概念的理解——不是背定义，是能画出一个 graph 并解释数据怎么流转。
  

  
    Checkpointer 解决什么问题？生产环境应该用哪种？
    考对"状态持久化"的理解——checkpoint 不只是存档，还是 HITL 和 time-travel debugging 的基础。
  

  
    LangGraph 怎么实现 Human-in-the-Loop？
    考 interrupt + checkpoint + resume 的三步机制。
  

  
    LangGraph vs CrewAI vs AutoGen，什么场景用哪个？
    考框架选型判断力——不是"LangGraph 最好"，而是各自的 sweet spot。
  

  
    LangGraph 的 Reducer 是什么？什么时候需要自定义 Reducer？
    考对并行写入同一 state 字段时冲突处理的理解。
  

  
    用 LangGraph 做一个 RAG + 条件路由的系统，画出 graph 结构。
    考实战能力——能在白板上画出节点、边、条件边的完整 graph。
