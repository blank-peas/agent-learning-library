...
> **资料来源**：[Agent Interview 100](https://github.com/BigKunLun/Agent-Interview-100) · 原文件：`07-prompt-engineering/063-prompt-chaining.md`。
...
flowchart TD
    subgraph LIN["线性链：直通"]
        A["提取实体"] -- "实体列表" --> B["分析关系"] -- "关系图谱" --> C["生成摘要"] -- "分析报告" --> D["格式化输出 → 最终文档"]
    end
    subgraph COND["条件链：按中间结果选路"]
        Q{"分类意图"} -- "查询" --> S["搜索 → 回答"]
        Q -- "操作" --> E["确认 → 执行"]
        Q -- "闲聊" --> X["直接对话"]
    end
    subgraph PAR["并行链：任务分解后三路扇出"]
        P1["子任务 A"] --> M["合并结果 → 最终输出"]
        P2["子任务 B"] --> M
        P3["子任务 C"] --> M
    end
    classDef proc fill:#e3f2fd,stroke:#1565c0,color:#0d47a1
    classDef decision fill:#fffde7,stroke:#f9a825,color:#795548
    class A,B,C,D,S,E,X,P1,P2,P3,M proc
    class Q decision
```

*链式三种拓扑：线性直通、条件分支选路、并行扇出-汇合。*
...
class PromptChain:
    ...
    async def run(self, initial_input):
        ...
        for step in self.steps:
            ...
            # 解析输出并加入上下文
            parsed = step["parser"](https://github.com/BigKunLun/Agent-Interview-100)
