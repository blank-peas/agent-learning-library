# MetaGPT 与 ChatDev 案例剖析

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`multi-agent/metagpt-chatdev.md`。
...
## MetaGPT 与 ChatDev 案例剖析
  ...
  2023 年同时出现的两个"AI 软件公司"项目，一个走结构化 SOP 路线、一个走两两对话路线。本文不只是讲它们怎么用，而是剖析它们解决了什么、留下了什么思想、为什么后来 Cognition 那篇博客要专门反对——把这两个项目放在 Multi-Agent 整个发展脉络里看。

  
    适合阶段：理解多 Agent 范式
    核心：SOP 结构化 vs Communicative 对话
    面试重点：辨析两种协作范式的取舍
...
> **本文边界**：本文是"案例深度剖析"。多 Agent 协作策略的综述请看 [Agent 协作策略](./chapters/05-编排与多agent/agent-camp-multi-agent-collaboration)；架构模式（hierarchical / network / supervisor）请看 [多 Agent 架构模式](./chapters/05-编排与多agent/agent-camp-multi-agent-patterns)；编程类 Agent 的横向综述请看 [编程 Agent](./chapters/09-codingagent/agent-camp-vertical-coding-agent)。本文聚焦在 **MetaGPT 和 ChatDev 这两个特定项目的设计选择与历史地位**。
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    MetaGPT 的 SOP 思想是什么？为什么它比纯自然语言对话稳定？
    考你能不能讲清"结构化 artifact 减少幻觉"这个机制。
  

  
    ChatDev 的 Communicative Dehallucination 是什么？两两对话为什么能减少幻觉？
    考你对"两 Agent 互相 review"这种纠错机制的理解。
  

  
    MetaGPT 和 ChatDev 哪个更适合生产？为什么？
    考你能不能从可控性、成本、可维护性反推架构选择。
  

  
    Cognition 2024 那篇 "Don't Build Multi-Agents" 的核心论点是什么？这两个项目怎么回应？
    考你看过业界反思，能客观评价多 Agent 范式的边界。
  

  
    为什么这两个开源项目影响了后来所有的多 Agent 框架？
    考你对"SOP + 角色扮演"这套思想如何被 AutoGen、CrewAI、CAMEL 继承的理解。
  

  
    MetaGPT 的 5 个角色为什么是这 5 个？少 1 个会怎样？多 1 个会怎样？
    考你对"角色边界设计"的判断——不是越多越好。
  

  
    ChatDev 的 Waterfall 流程在 LLM 场景下有什么独特价值？
    考你能不能跳出"瀑布过时"的刻板印象，理解它在 Agent 场景的合理性。
  

  
    如果让你自己做一个"AI 软件公司"，你会怎么改进 MetaGPT/ChatDev 的设计？
    考开放性思维 + 对现有方案缺陷的洞察。
...
CrewAI 等于"把 MetaGPT 的 SOP 思想做成更通用的 Python SDK"。可结合 [框架选型对比](./chapters/07-ts产品工程/agent-camp-frameworks-comparison) 看它在多 Agent 框架里的定位。
...
- **AutoGen** 的 GroupChat 模式本质是 N 个 Agent 的对话池，加上 `select_speaker` 函数决定谁说话。如果只有 2 个 agent，就退化成 ChatDev 风格。可结合 [框架选型对比](./chapters/07-ts产品工程/agent-camp-frameworks-comparison) 看它和其它框架的差异。
...
| 概念 | 范围 | 本文位置 |
|---|---|---|
| **多 Agent 协作策略** | 综述：竞争 / 协作 / 辩论 / 投票 | [collaboration](./chapters/05-编排与多agent/agent-camp-multi-agent-collaboration) |
| **多 Agent 架构模式** | 综述：hierarchical / network / supervisor | [patterns](./chapters/05-编排与多agent/agent-camp-multi-agent-patterns) |
| **Orchestrator-Worker 模式** | 单层调度模型 | [orchestrator-worker](./chapters/05-编排与多agent/agent-camp-multi-agent-orchestrator-worker) |
| **编程 Agent 综述** | 横向看 Devin / Cursor / Aider / Claude Code | [coding-agent](./chapters/09-codingagent/agent-camp-vertical-coding-agent) |
| **CrewAI 框架** | 受 MetaGPT 启发的多 Agent SDK | [frameworks comparison](./chapters/07-ts产品工程/agent-camp-frameworks-comparison) |
| **AutoGen 框架** | 微软的 GroupChat 多 Agent 框架 | [frameworks comparison](./chapters/07-ts产品工程/agent-camp-frameworks-comparison) |
...
- **配套阅读**：
  - [多 Agent 协作策略](./chapters/05-编排与多agent/agent-camp-multi-agent-collaboration) — 协作范式综述（竞争 / 协作 / 辩论 / 投票）
  - [多 Agent 架构模式](./chapters/05-编排与多agent/agent-camp-multi-agent-patterns) — hierarchical / network / supervisor 模式
  - [Orchestrator-Worker 模式](./chapters/05-编排与多agent/agent-camp-multi-agent-orchestrator-worker) — 单层调度的最简模型
  - [编程 Agent](./chapters/09-codingagent/agent-camp-vertical-coding-agent) — Devin / Cursor / Aider / Claude Code 横向对比
  - [框架选型对比](./chapters/07-ts产品工程/agent-camp-frameworks-comparison) — CrewAI / AutoGen 等多 Agent 框架的定位对比
