# LangChain / LangGraph 深度剖析

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`frameworks/langchain.md`。
...
## LangChain / LangGraph 深度剖析
  ...
  LangChain 是 LLM 应用框架里最被"恨爱交织"的——抽象层太厚被骂，但生态大到没有替代品。2024 年它把 Agent 能力单独拆成 LangGraph，从"链式编排"转向"状态机编排"，这是 Agent 工程化的重要转折。本文讲清两者的边界，以及什么场景该选哪个。

  
    适合阶段：入门 / 进阶 / 生产
    核心链路：Runnable → Chain → Agent / Graph
    面试重点：LCEL + Graph 状态机 + checkpointer
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    LangChain 和 LangGraph 是什么关系？什么时候用哪个？
    考对生态演进的理解——LangChain 做基础抽象（LLM/Prompt/Tool），LangGraph 做 Agent 编排，两者互补不替代。
  

  
    LCEL 解决了什么问题？它和普通 Python 函数调用比有什么优势？
    考对声明式编程的认知——pipe 操作符、自动并行、流式、batch、async 全免费拿到。
  

  
    LangChain 的 AgentExecutor 为什么被 LangGraph 取代？
    考架构理解——AgentExecutor 是黑盒 while 循环，状态不可见、不可回放、不可干预；Graph 把每步拆成节点，全程可控。
  

  
    LangGraph 的 State 和 Reducer 是什么概念？为什么这样设计？
    考状态管理的核心——多节点并发更新同一份 State 时如何合并，Reducer 决定合并策略。
  

  
    Checkpointer 是怎么实现 Agent 的"暂停 / 恢复 / 时间旅行"的？
    考持久化机制——每个超级步骤后保存 State 快照到数据库，重启时从最近 checkpoint 恢复。
  

  
    Human-in-the-loop 在 LangGraph 里怎么做？和直接在循环里加 input() 有什么不同？
    考异步中断模型——interrupt() 把节点执行挂起、持久化状态，可以隔天回来继续。
  

  
    LangChain 经常被骂"抽象太厚"，你怎么看？什么场景下应该弃用？
    考工程取舍判断——简单 RAG/Chat 直接用 SDK 更轻；复杂多 Agent 场景 LangGraph 仍然是最稳的选择。
