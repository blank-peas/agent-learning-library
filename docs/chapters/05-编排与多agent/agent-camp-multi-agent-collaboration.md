# Agent 协作策略 Debate / Voting / Role-Play / Reviewer

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`multi-agent/collaboration.md`。
...
## Agent 协作策略 Debate / Voting / Role-Play / Reviewer
  ...
  单 Agent 容易钻牛角尖、自信且错误。四种协作模式让多个 Agent 互相质疑、投票、分工、审查——比单 Agent 的 self-refine 更客观，比 self-consistency 多了"分歧建模"的能力。

  
    适合阶段：多 Agent 进阶 / 生产架构选型
    核心：四种正交协作模式 + 成本与收敛性权衡
    面试重点：场景匹配 + 不收敛兜底 + 与单 Agent 反思的区别
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    Debate / Voting / Role-Play / Reviewer 四种模式各适合什么场景？
    考你能不能按"任务类型 × 答案空间 × 成本预算"做匹配，而不是堆术语。
  

  
    多 Agent debate 一直不收敛（两个 Agent 互相杠）怎么办？
    考你知不知道 max_rounds、judge 兜底、温度退火这些工程兜底策略。
  

  
    Reviewer 模式和 Self-Refine 的区别在哪？为什么前者更客观？
    考能不能讲清"独立 Agent 视角"和"同一模型自审"的本质差别。
  

  
    投票时 N 个 Agent 该用同模型还是不同模型？
    考对 ensemble diversity 的理解——同模型只是降方差，跨模型才能降偏差。
  

  
    Role-Play 的角色边界怎么定义？怎么防止角色"穿帮"互相干涉？
    考 prompt 设计、消息隔离、责任边界的工程经验。
  

  
    多 Agent debate 真的能比 self-consistency 更好吗？提升来自哪？
    考你读没读过 Du et al. 2023，能不能讲清"分歧暴露错误"这个机制。
  

  
    协作 N 个 Agent，成本是单 Agent 的 N 倍——怎么控制？
    考成本意识：早停、分层、cache、便宜模型当陪练等。
  

  
    RLAIF / Constitutional AI pipeline 里 reviewer 角色起到什么作用？
    考对"协作不仅是 inference 时的事，也是训练 pipeline 的事"的理解。
