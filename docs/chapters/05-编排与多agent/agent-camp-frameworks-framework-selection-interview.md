# Agent 框架选型面试深挖：自研薄封装不是重新造 LangChain

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`frameworks/framework-selection-interview.md`。
...
## Agent 框架选型面试深挖：自研薄封装不是重新造 LangChain
  ...
  面试官问“为什么不用框架”，不是在考你会不会报框架名，而是在考你的工程边界感。

  
    适合阶段：Agent 工程师 / 架构面
    核心能力：边界成本 · 控制权 · 迁移路径 · 框架锁定
    脱敏原则：只保留选型方法，不保留真实业务细节
...
这组题最容易把“会用框架”和“能做架构取舍”区分开。
    ...
    你为什么没有直接用 LangChain / LangGraph？
    考你是否能说明框架的边界成本，而不是只说“功能简单”。
    ...
    什么场景下你会从自研切到 LangGraph？
    考复杂状态机、checkpoint、human-in-the-loop 和长期任务判断。
  

  
    OpenAI Agents SDK 和 LangGraph 怎么选？
    考 handoff 线性分流和 graph 状态编排的边界。
  

  
    框架帮你省了哪些代码，又强加了哪些约束？
    考 framework fit，不是功能清单。
  

  
    如果后面需求复杂了，自研封装迁移成本怎么控制？
    考抽象隔离、接口稳定和渐进迁移。
  

  
    怎么证明你的自研封装质量不比框架差？
    考测试边界、trace、eval、故障回放和团队可维护性。
  

  
    低代码 Agent 平台、SDK 框架、裸 SDK 三者怎么选？
    考团队协作、上线速度、可控性和供应商锁定。
