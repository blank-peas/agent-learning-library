# Agent Harness 设计：让模型稳定做长任务的运行环境

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`engineering/harness.md`。
...
## Agent Harness 设计：让模型稳定做长任务的运行环境
  ...
  模型负责判断下一步，harness 负责让任务可观察、可验证、可恢复。

  
    适合阶段：生产 Agent / 长周期任务
    核心机制：Task State + Tool Loop + Checkpoint
    面试重点：Harness vs Prompt vs Workflow
...
### 面试官想考什么
    ...
    Harness 和 prompt、workflow、agent loop 分别差在哪？
    考你能不能把执行环境和模型指令分开。
  

  
    为什么长周期 Agent 不能只靠长上下文或自动压缩？
    考状态外置、检查点和恢复策略。
  

  
    一个 coding agent harness 至少要提供哪些能力？
    考文件、命令、测试、日志、diff、权限、回滚。
  

  
    Harness 怎么防止目标漂移？
    考任务契约、验收标准、阶段性检查和用户确认。
  

  
    工具返回大段日志时，harness 应该怎么处理？
    考 observation 设计和上下文污染控制。
  

  
    Harness、MCP、Skill、Sandbox 这些层怎么配合？
    考平台级分层能力。
  

  
    怎么评估一个 harness 是否比另一个更好？
    考除了模型分数以外的环境变量控制。
