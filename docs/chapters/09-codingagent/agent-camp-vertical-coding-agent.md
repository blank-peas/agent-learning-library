...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`vertical/coding-agent.md`。
...
## 编程 Agent
  ...
  2024-2026 年最快爆发的 Agent 形态。从 GitHub Copilot 的"代码补全"到 Claude Code/Cursor 的"自主完成 PR"，再到 Devin/OpenHands 的"端到端解决 Issue"，编程 Agent 一路把"AI 能写代码"推进到"AI 能维护代码库"。本文讲清楚：编程 Agent 区别于通用 Agent 的核心技术决策，以及为什么这一代产品趋同地选了同一套工具集。

  
    适合阶段：进阶 / 工程实战
    核心链路：定位 → 修改 → 验证 → 自纠
    面试重点：工具集设计 + SWE-bench + 多文件协同
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    编程 Agent 和通用 Agent 的核心区别是什么？为什么不能用 LangChain 通用模板写一个就完事？
    考对垂直化设计的理解——编程场景的工具集（精确编辑、文件系统、shell）、约束（多文件一致性、可验证性）、失败模式都和通用 Agent 不同。
  

  
    为什么 Claude Code/Cursor/Cline/Aider 的工具集都长得越来越像？
    考工程收敛性——这套工具集（Read/Edit/Bash/Glob/Grep）是经过两年迭代验证的最小完备集，背后是 SWE-bench 等 benchmark 推动的实证选择。
  

  
    Agent 怎么在百万行代码库里找到要改的位置？靠 RAG？靠语义搜索？
    考 agentic search 的认知——不是预先嵌入整个仓库，而是 Agent 主动用 Grep/Glob 探索，本质是把"检索"任务交给 LLM 而非检索系统。
  

  
    Plan mode 和 Act mode 为什么要分开？模型能自己判断什么时候 plan 什么时候 act 吗？
    考 HITL 边界设计——分离不是因为模型笨，是因为人需要在 plan 阶段对齐意图，避免 Agent 改完一堆文件才发现方向错了。
  

  
    Edit 工具为什么要强制"必须先 Read"和"old_string 唯一"这些约束？换成 fuzzy match 不更方便吗？
    考 poka-yoke 设计哲学——把 LLM 易错的场景设计成"错误根本无法发生"，而不是事后纠错。
  

  
    SWE-bench 上的分数能反映编程 Agent 的真实能力吗？提分背后的 trick 有哪些？
    考 benchmark 批判性——SWE-bench 是单 Issue 修复场景，和真实工程（设计讨论、跨团队对齐、长期维护）差距巨大。
  

  
    多 Agent 协作（写代码 + code review + 跑测试）真的比单 Agent 强吗？
    考"多 Agent" hype 的辨析——多数场景单 Agent + 工具链就够，多 Agent 引入的协调开销往往大过收益。
  

  
    编程 Agent 在生产里最常见的失败模式是什么？怎么缓解？
    考生产经验——常见失败：context 满了忘事、误改无关代码、循环重试、被 prompt injection 攻击。每种失败有对应的设计缓解。
