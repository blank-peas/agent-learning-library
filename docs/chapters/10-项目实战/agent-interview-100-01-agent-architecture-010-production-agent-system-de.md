...
> **资料来源**：[Agent Interview 100](https://github.com/BigKunLun/Agent-Interview-100) · 原文件：`01-agent-architecture/010-production-agent-system-design.md`。
...
```mermaid
flowchart TD
    subgraph G["用户接入层 Gateway"]
        G1["Web / App / 微信 / API 开放接口"]
        G2["统一消息协议 Protobuf"]
    end
    subgraph O["Agent 编排层 Orchestrator"]
        O1["意图路由器"]
        O2["对话状态机"]
        O3["工具调度器"]
        O4["Handoff 判断器 · Guard Rails"]
    end
    subgraph I["LLM 推理层 Inference"]
        I1["模型路由器<br/>GPT-4 级 / 轻量模型"]
        I2["语义缓存"]
        I3["限流器 Rate Limiter"]
    end
    subgraph D["数据层 Data Store"]
        D1["向量知识库 Milvus/PG<br/>工单系统 MySQL/PG"]
        D2["用户画像 Redis/ES<br/>对话日志 MongoDB"]
        D3["分析数据仓库 ClickHouse"]
    end
    G --- O --- I --- D
    classDef proc fill:#e3f2fd,stroke:#1565c0,color:#0d47a1
    classDef agent fill:#f3e5f5,stroke:#6a1b9a,color:#4a148c
    classDef store fill:#e8f5e9,stroke:#2e7d32,color:#1b5e20
    class G1,G2,O1,O2,O3,O4 proc
    class I1,I2,I3 agent
    class D1,D2,D3 store
```
*智能客服四层架构：接入层统一协议，编排层路由与状态，推理层模型路由与缓存，数据层多存储分工。*
...
```mermaid
flowchart TD
    GW["1 Gateway 接收<br/>协议转换 · 鉴权 · 限流"] --> SS["2 加载 Session<br/>Redis"]
    SS --> IR["3 意图路由<br/>BERT 约 15ms / LLM 约 500ms"]
    IR --> HD{"4 需要转人工?"}
    HD -- "是" --> HO["Human Handoff<br/>转人工客服"]
    HD -- "否" --> SC{"5 语义缓存命中?"}
    SC -- "命中" --> RET["返回响应<br/>总延迟 P95 低于 2.5s"]
    SC -- "未命中" --> GEN["6 RAG / LLM 生成<br/>800-2000ms"]
    GEN --> GR["7 Guard Rails 检查<br/>安全过滤 + 质量校验"]
    GR --> UPD["8 更新 Session<br/>+ 异步写日志"]
    UPD --> RET
    classDef proc fill:#e3f2fd,stroke:#1565c0,color:#0d47a1
    classDef decision fill:#fffde7,stroke:#f9a825,color:#795548
    classDef neutral fill:#eceff1,stroke:#546e7a,color:#37474f
    class GW,SS,IR,GEN,GR,UPD,RET proc
    class HD,SC decision
    class HO neutral
```
*一次请求 8 步全链路：语义缓存命中直接返回，Handoff 触发转人工，总延迟 P95 < 3 秒。*
