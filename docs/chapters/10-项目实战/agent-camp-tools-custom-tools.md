...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`tools/custom-tools.md`。
...
## 自定义工具开发实战
  ...
  写一个工具不是定义一个 Python 函数那么简单。从"模型该不该用、什么时候用"到"调错参数怎么 debug、上线后怎么灰度"——本文把六个阶段串起来，给你一份能直接抄的端到端工作流。

  
    适合阶段：Agent 生产化
    核心：需求 → 设计 → schema → 实现 → 测试 → 上线
    面试重点：从工具开发讲到工程素养
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    一个工具能力很大，是拆成多个小工具还是合并成一个大工具？怎么判断？
    考工具粒度设计——是否理解"模型选择负担 vs 参数膨胀"的取舍。
  

  
    怎么测试 LLM 是否能正确使用你的工具？单元测试够吗？
    考"LLM 集成测试"的概念——是否做过评估集驱动的工具开发。
  

  
    schema 改动后线上 agent 不兼容，怎么处理？
    考工具版本管理——deprecate / 兼容期 / 双跑策略。
  

  
    工具上线后发现模型 60% 调错参数，你怎么 debug？
    考观测 + 归因能力——能不能从 trace 反推 schema 问题。
  

  
    本地函数 / HTTP API wrapper / MCP server——同一个工具这三种形态选哪个？
    考工具形态的工程权衡：跨进程、跨语言、跨团队的边界。
  

  
    工具内部要不要带状态？比如分页 cursor、session id 这种？
    考有状态 vs 无状态工具的取舍。
  

  
    工具描述（description）怎么写模型才知道什么时候该用？
    考 prompt 与 schema 的交界——description 是给模型读的，不是给同事读的。
  

  
    上线后怎么知道这个工具到底有没有被正确使用？要埋哪些点？
    考工具可观测性——调用频次、参数分布、错误率、回放能力。
