# 记忆架构 Memory（MemGPT / Mem0）

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`agent/memory-arch.md`。
...
## 记忆架构 Memory（MemGPT / Mem0）
  ...
  Agent 记忆要把可复用事实、偏好、经验和技能分层管理。

  
    适合阶段：长任务与个性化 Agent
    核心机制：Working / Episodic / Semantic / Procedural
    面试重点：写入、检索、过期和隐私
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    短期记忆、长期记忆、情景记忆、语义记忆分别是什么？
    考 memory taxonomy，不接受“都放向量库”。
  

  
    MemGPT 为什么用操作系统类比？
    考 context window、外部存储、分页和控制流。
  

  
    记忆什么时候写入？谁来决定写入？
    考 memory write policy 和人工确认。
  

  
    记忆检索怎么避免把旧信息或错误信息拿回来？
    考时间、置信度、命名空间、冲突解决。
  

  
    长期记忆会带来哪些安全和隐私问题？
    考 consent、PII、删除权、权限隔离。
  

  
    怎么评估一个 memory system？
    考 recall、precision、staleness、contradiction、task success。
