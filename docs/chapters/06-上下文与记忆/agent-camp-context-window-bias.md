...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`context/window-bias.md`。
...
## 上下文窗口与位置偏置
  ...
  Lost in the Middle——模型对 prompt 中间位置的信息会"看不见"，准确率掉 20%+。理解这个现象决定了所有上下文设计的方向。
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    Lost in the Middle 是什么？实测掉点幅度多少？
    考具体数据，能不能讲清现象的严重程度。
  

  
    为什么注意力会有 U 形分布？根因是什么？
    考对 RoPE / causal mask / 训练数据偏好的理解。
  

  
    长上下文模型（Gemini 2M、Claude 200K）是否解决了这个问题？
    考你跟得上 2024 后的实测，不能只信厂商宣传。
  

  
    Needle in a Haystack 测试是什么？为什么它过了不代表长上下文真的好用？
    考评估方法论。
  

  
    实战中怎么对抗 Lost in the Middle？
    考工程对策，不能只说"放开头或末尾"。
  

  
    RAG top-K 的 K 选 5 还是 20？和 Lost in the Middle 有关吗？
    考 RAG 和上下文偏置的交叉。
  

  
    Chat history 越积越长会不会让早期 system prompt 失效？
    考多轮对话的位置偏置。
