# Agent 工程异常处理面试深挖：RAG 打架、记忆冲突、规划死循环怎么办

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`engineering/agent-failure-modes-interview.md`。
...
## Agent 工程异常处理面试深挖：RAG 打架、记忆冲突、规划死循环怎么办
  ...
  面试官问“资料互相矛盾怎么办”，不是在等一个标准答案，而是在看你有没有工程裁决机制。

  
    适合阶段：Agent 工程师 / 平台架构面
    核心能力：冲突归因 · 停止条件 · 证据裁决 · 回归闭环
    脱敏原则：只保留工程方法，不保留真实业务细节
...
这组题没有一个“背下来就满分”的标准答案。面试官看的是你能不能把混乱问题拆成可执行决策。
    ...
    RAG 召回了多篇文档，但内容互相矛盾，你让模型怎么答？
    考证据分组、时效、来源可信度、引用和澄清，而不是“让模型自己判断”。
  

  
    长期记忆说用户偏好 A，当前对话里用户又说要 B，谁优先？
    考 memory 的 scope、source、confidence、TTL 和覆盖机制。
  

  
    检索出来的资料有旧版、新版和噪声，怎么避免拼成一个混合答案？
    考 stale evidence、版本过滤、answer abstention 和冲突标注。
  

  
    Agent 一直在规划、重规划，不进入执行，怎么办？
    考规划死循环的停止条件、replan budget、no-progress detector 和人工/澄清分支。
  

  
    怎么区分 retry、repair、replan、refuse？
    考失败分类：瞬时错误、参数错误、前提变化和安全/权限边界不能混着处理。
  

  
    如果没有足够证据，Agent 应该继续检索、提问澄清，还是直接回答“不知道”？
    考 evidence policy 和用户体验，不是盲目扩大 top-k。
  

  
    这类异常怎么进评估集？怎么防止修一个 case 又引入新问题？
    考 conflict slice、loop slice、case-level diff 和回归门禁。
  

  
    如果你现在重做，会把这些异常处理沉到哪一层？
    考平台化抽象：retrieval evaluator、memory resolver、planner guard、runtime policy。
