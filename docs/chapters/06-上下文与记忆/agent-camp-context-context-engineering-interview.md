# Context Engineering 面试深挖：上下文不是越多越好

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`context/context-engineering-interview.md`。
...
## Context Engineering 面试深挖：上下文不是越多越好
  ...
  面试官追问的不是“你有没有做摘要”，而是“你怎么保证模型看见正确的东西”。
...
这些题听起来像细节题，其实是在考你有没有把 context 当成一个工程系统来管理。
    ...
    你说做了 context engineering，具体控制了哪些东西？
    考你能不能从 prompt 技巧上升到输入预算、布局和运行时策略。
  

  
    长上下文没到窗口上限，为什么还要压缩？
    考你是否知道压缩有硬触发和软触发，也知道压缩会带来信息损失。
  

  
    摘要怎么保证“符合原文”？出错后能回溯吗？
    考摘要保真、message 序号、关键事实结构化、原文可回放。
  

  
    压缩会不会让 prompt cache / prefix cache 失效？怎么布局？
    考稳定前缀、动态区域、cache 命中和 token 减少之间的权衡。
  

  
    RAG 召回内容拿到后，直接拼进 prompt 可以吗？
    考资料角色隔离、去重、排序、citation、冲突处理和位置策略。
  

  
    history、memory、RAG、tool definitions 都在抢窗口，怎么分预算？
    考你是否能把上下文当作有限资源，而不是谁都无限塞。
  

  
    除了摘要，还有哪些压缩方式？子 Agent 委派算不算压缩？
    考方法库边界：rolling summary、结构化抽取、检索式上下文、外部状态。
  

  
    上下文污染、旧事实、错误工具结果进入 prompt 后怎么清理？
    考 context hygiene，不只是 context packing。
