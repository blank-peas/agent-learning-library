# Memory Governance 面试深挖：别把所有历史都塞进向量库

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`context/memory-governance-interview.md`。
...
## Memory Governance 面试深挖：别把所有历史都塞进向量库
  ...
  面试官追问长期记忆，真正想听的是一套可控的写入、召回、覆盖、删除和评估机制。

  
    适合阶段：Agent 工程师 / 平台面
    核心能力：写入策略 · 冲突消解 · TTL · 权限 · 召回
    脱敏原则：只保留技术追问，不保留项目细节
...
这些题表面在问 memory，实际在考你有没有把“长期状态”当成生产系统来治理。
    ...
    Agent memory 和 chat history、RAG 的边界在哪里？
    考你能不能分清原始日志、用户状态、外部知识库三类信息。
  

  
    哪些内容应该写入长期记忆？写入动作由谁触发？
    考 write policy，不接受“模型觉得重要就存”。
  

  
    一条 memory record 至少要带哪些治理字段？
    考 schema 意识：source、confidence、TTL、scope、status、permission。
  

  
    新旧记忆冲突时怎么覆盖？会不会保留历史版本？
    考版本、置信度、作用域和审计，不只是 upsert。
  

  
    怎么防止 prompt injection 或幻觉写入长期记忆？
    考 memory pollution，能不能把模型提议和代码裁决分开。
  

  
    记忆很多时怎么按需召回？直接全量塞 profile 可以吗？
    考 memory-as-tool、过滤、重排和上下文预算。
  

  
    用户如何查看、删除、禁用记忆？多租户权限怎么隔离？
    考隐私、合规和平台边界。
  

  
    怎么评估 memory system？怎么防一次改动把旧记忆重新激活？
    考 write precision、retrieval precision、stale rate、回归集。
