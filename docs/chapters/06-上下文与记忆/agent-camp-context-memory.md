...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`context/memory.md`。
...
## Agent 记忆系统
  ...
  同一个用户上周说过他对花生过敏，今天他点外卖你能不能想起来——这就是 Agent 的记忆问题。它和"会话历史"是两件事。

  
    适合阶段：Agent 进阶 / 生产
    核心：跨 session 的持久化记忆架构
    面试重点：分层 + 写入策略 + 召回
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    Agent 的 memory 和 chat history 有什么区别？
    考概念分层，看你能不能分清"短期 / 长期 / 会话历史"三个东西。
  

  
    episodic、semantic、procedural memory 分别是什么？各放哪里？
    考认知科学迁移到工程的能力，是否照搬过 CoALA 论文的分类。
  

  
    什么时候 commit 一条信息到长期记忆？谁来决定？
    考写入策略的取舍——LLM 自决 vs 触发式 vs 显式工具。
  

  
    MemGPT 的"操作系统类比"具体怎么落地的？
    考对论文的真实阅读，能不能讲清 main context / external context 怎么切换。
  

  
    mem0、Letta、LangMem、Zep 这几家路线区别在哪？
    考主流方案选型能力，2025 面试高频。
  

  
    多用户共用同一个 memory store 怎么做隔离？memory 中毒是什么？
    考生产经验和安全意识。
  

  
    记忆条目越积越多，召回质量怎么保持？怎么去重和遗忘？
    考长期运行 Agent 的工程细节。
  

  
    检索回来的 memory 应该塞进 system prompt 还是 user message？
    考对 instruction hierarchy 和位置偏置的理解。
