...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`rag/chunking.md`。
...
## 文档切分策略 Chunking
  ...
  让 RAG 检索命中完整证据，而不是把答案切坏、把噪声塞进 prompt。
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    chunk size 选 256、512 还是 1024？你怎么证明这个选择是对的？
    考工程判断，不接受背默认值。
  

  
    chunk overlap 设大一点是不是一定更好？
    考你是否理解边界召回、索引膨胀和重复上下文。
  

  
    为什么很多 RAG demo 用 RecursiveCharacterTextSplitter，生产系统却不能只靠它？
    考文档结构意识，尤其是 Markdown、PDF、表格、代码。
  

  
    小 chunk 检索准，大 chunk 上下文足，这个矛盾怎么解？
    考 parent-child retrieval / chunk expansion。
  

  
    语义切分听起来更聪明，为什么它有时比固定切分还差？
    考对实验结论和局部贪心问题的理解。
  

  
    中文、日文这类没有空格边界的文本，默认 splitter 会出什么问题？
    考细节经验，能不能讲到 separators 和 tokenizer。
  

  
    长上下文模型已经能吃 128K token，还需要 chunking 吗？
    考 RAG 与长上下文的边界，以及 Lost in the Middle。
