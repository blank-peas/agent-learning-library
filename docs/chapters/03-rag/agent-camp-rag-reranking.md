...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`rag/reranking.md`。
...
## 重排序 Reranking
  ...
  先把可能相关的 chunk 捞上来，再用更贵但更准的模型把真正能回答问题的证据排到前面。

  
    适合阶段：Hybrid Search 之后
    核心机制：粗召回 → 精排 → 入 prompt
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    RAG 为什么需要 reranking？第一阶段检索不是已经排过序了吗？
    考召回和精排的职责边界。
  

  
    bi-encoder 和 cross-encoder 的区别是什么？为什么 cross-encoder 更准但更慢？
    考模型结构，不接受“更智能”这种虚答。
  

  
    典型 reranking pipeline 怎么设计？top-50、top-100 怎么选？
    考候选数、延迟、召回之间的取舍。
  

  
    reranker 输入整篇文档还是 chunk？长文档被截断怎么办？
    考 token 上限和 chunk 粒度。
  

  
    BGE-reranker、Cohere Rerank、Jina、Voyage 这类方案怎么选？
    考开源部署、托管 API、多语言和成本。
  

  
    ColBERT / late interaction 和普通 cross-encoder reranker 有什么区别？
    考检索模型谱系，不是所有 reranker 都同一种。
  

  
    LLM-as-reranker 值不值得用？RankGPT 这类方法有什么工程问题？
    考前沿理解和成本意识。
