...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`rag/agentic-rag.md`。
...
## Agentic RAG
  ...
  固定 RAG 流水线解决不了多源、多跳、需要判断下一步的问题时，把检索变成 Agent 可调用的工具。

  
    适合阶段：复杂问答与多源检索
    核心机制：Plan / Tool / Observe / Stop
    面试重点：可控性与评估
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    Agentic RAG 和普通 RAG 最大区别是什么？
    考固定 pipeline 和动态 tool loop 的差异。
  

  
    什么场景值得上 Agentic RAG？什么场景不该上？
    考工程克制，不是所有问答都需要 Agent。
  

  
    Agentic RAG 的典型循环怎么设计？如何避免无限检索？
    考 max steps、stop condition、evidence grading。
  

  
    检索工具应该怎么设计 schema？返回整段文本还是结构化证据？
    考工具契约和可引用性。
  

  
    Agent 什么时候应该重写 query、换检索源、或者拒答？
    考分支控制，而不是无脑 tool calling。
  

  
    Agentic RAG 怎么评估？为什么比普通 RAG 更难测？
    考轨迹评估、工具调用准确率和端到端质量。
  

  
    Agentic RAG 和 GraphRAG、Self-RAG、ReAct 是什么关系？
    考概念辨析和架构分层。
