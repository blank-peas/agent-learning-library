# RAG 选型面试深挖：别再说“当时就用了”

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`rag/rag-selection-interview.md`。
...
## RAG 选型面试深挖：别再说“当时就用了”
  ...
  RAG 选型题考的不是你用过哪个平台，而是你有没有一套能经得起追问的决策框架。

  
    适合阶段：Agent 工程师 / 平台面 / 项目深挖
    核心能力：托管 vs 自建 · recall@k · rerank · 退出条件
    脱敏原则：只保留技术取舍，不保留项目细节
...
这组问题的杀伤力很强，因为它能把“做过一个 RAG demo”和“能为生产系统做技术判断”直接分开。
    ...
    你们为什么选托管知识库平台，而不是自建 Milvus / pgvector / Elasticsearch？
    考你能不能讲交付周期、运维成本、可控性、评估指标和退出条件。
  

  
    如果当时没有做横向对比，面试里怎么诚实回答？
    考成熟度：承认阶段性选择，同时补上现在会怎么验证。
  

  
    RAG 方案选型要看哪些指标？只看最终答案行不行？
    考 retrieval、grounding、answer quality、latency、cost 的分层意识。
  

  
    什么时候用 pgvector，什么时候上专用向量库？
    考规模、过滤、更新、运维复杂度和团队能力之间的取舍。
  

  
    为什么生产 RAG 往往要混合检索和 rerank？
    考你是否知道精确词、错误码、专有名词靠 BM25 更稳，语义改写靠 dense 更稳。
  

  
    TopK、相似度阈值、chunk size 怎么调？
    考你有没有用评估集调参，而不是在控制台凭感觉拖滑块。
  

  
    如果后面从托管迁到自建，怎么保证效果不回退？
    考 index version、case-level diff、shadow run 和灰度切换。
  

  
    权限、审计、数据驻留会怎么影响 RAG 选型？
    考多租户和合规意识：检索层过滤不是 UI 层过滤。
