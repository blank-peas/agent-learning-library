...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`rag/hybrid-search.md`。
...
## 混合检索 Hybrid Search
  ...
  让关键词精确匹配和语义向量召回互相补位，别让 RAG 输在第一轮候选。
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    BM25 解决什么问题？为什么不是词频越高越相关？
    考 IDF、词频饱和、文档长度归一化。
  

  
    Dense retrieval 和 BM25 的根本差异是什么？
    考语义召回和词项匹配的边界。
  

  
    为什么 RAG 里常用 Hybrid Search？
    考精确术语和语义改写两类 query 的互补。
  

  
    Hybrid Search 是分数相加，还是结果融合？
    考 score normalization、rank fusion、RRF 的区别。
    ...
    RRF 的公式是什么？`k=60` 是什么意思？
    考是否真懂融合，而不是只会调库。
  

  
    Hybrid Search 后还需要 reranker 吗？
    考召回和精排的职责边界。
