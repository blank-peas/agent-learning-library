...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`rag/graphrag.md`。
...
## GraphRAG 图增强检索
  ...
  当问题需要理解一批文档里的实体、关系和主题时，GraphRAG 才值得上场。

  
    适合阶段：复杂知识库分析
    核心机制：实体 / 关系 / 社区摘要
    面试重点：Local vs Global Search
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    GraphRAG 解决的是什么问题？普通向量 RAG 为什么不够？
    考你是否知道它主要面向全局理解和多跳关系。
  

  
    Microsoft GraphRAG 的索引阶段做了哪些事？
    考实体抽取、关系抽取、社区检测、community report。
    ...
    GraphRAG 一定比 Vector RAG 好吗？什么时候反而更差？
    考成本、抽取错误和任务匹配。
  

  
    实体消歧为什么是 GraphRAG 的关键难点？
    考真实工程问题：同名、别名、缩写、跨文档合并。
  

  
    GraphRAG 的成本主要花在哪里？怎么降？
    考 LLM 抽取、社区摘要、增量更新和 LazyGraphRAG。
  

  
    GraphRAG 和知识图谱 QA 有什么区别？
    考是否能区分“临时从文本构图”和“长期治理的 KG”。
