# 嵌入模型选型 Embedding Models

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`rag/embedding-models.md`。
...
## 嵌入模型选型 Embedding Models
  ...
  RAG 召回质量的第一道闸门：别只看排行榜，要用自己的问题、语料和成本约束选模型。
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    RAG 里选 embedding 模型，你看 MTEB 总分还是先做自己的 retrieval eval？
    考评估意识，排行榜只是候选筛选器。
  

  
    text-embedding-3-small 和 text-embedding-3-large 怎么取舍？
    考维度、价格、召回质量、向量库成本的整体判断。
  

  
    为什么 BGE、E5、Cohere 这类模型要区分 query 和 document？
    考 asymmetric retrieval，不是所有文本都该同样编码。
  

  
    embedding 维度越大越好吗？降维会不会毁掉召回？
    考 Matryoshka / dimensions 参数和向量库成本。
  

  
    为什么线上换 embedding 模型不能只改一行模型名？
    考索引重建、双写、回滚和向量空间不兼容。
  

  
    多语言、代码、法律/金融知识库，为什么通用模型可能不够？
    考领域迁移和专用评测集。
  

  
    dense embedding、BM25、hybrid search、reranker 各自解决什么问题？
    考流程分工，避免把所有锅都甩给 embedding。
