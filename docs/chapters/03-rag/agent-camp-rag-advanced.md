...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`rag/advanced.md`。
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    Naive RAG 已经有 chunking、embedding、reranking，为什么还需要高级 RAG？
    考你能不能识别“召回失败”和“生成失败”的边界。
  

  
    HyDE 为什么能提升 zero-shot dense retrieval？它最大的风险是什么？
    考你是否理解“假想答案”是检索 query，不是事实来源。
  

  
    Multi-query / RAG-Fusion 和普通 query rewrite 有什么区别？
    考多路召回、RRF 和候选池扩展。
  

  
    Step-back prompting 适合解决什么 RAG 问题？
    考抽象问题和具体问题的互补。
  

  
    Self-RAG 和 CRAG 都在“自检”，它们差别在哪里？
    考论文机制，而不是把名字混成一团。
    ...
    高级 RAG 的效果怎么评估？只看答案准确率够吗？
    考分段评估：改写、召回、重排、生成、拒答。
