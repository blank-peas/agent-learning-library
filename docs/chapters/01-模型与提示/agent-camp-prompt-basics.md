...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`prompt/basics.md`。
...
## 提示词基础原则
  ...
  所有高级提示技巧（CoT、Few-shot、ReAct）都建立在四个基础原则上——先把它们写对，再谈别的。
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    为什么"请帮我写一篇文章"是糟糕的 prompt？
    考你能不能讲清"歧义性"如何导致模型输出失控。
  

  
    结构化 prompt（XML / Markdown）和自然语言堆砌差别在哪？
    考对模型注意力机制的理解。
  

  
    role prompting（"你是一个资深律师"）到底有没有用？
    陷阱题，2024 后的研究结论是"在指令模型上效果微弱甚至有害"。
  

  
    同样的任务，prompt 写长了还是写短了好？
    考权衡，没有绝对答案的题。
  

  
    怎么让模型"严格遵守输出格式"？为什么有时候它就是不听？
    考工程经验，区分纸上知识和实战。
  

  
    negative prompt（"不要做 X"）为什么经常失效？
    考对 LLM 注意力偏向的认知。
  

  
    怎么判断一个 prompt 的好坏？有没有量化方法？
    考评估意识，不能凭感觉调 prompt。
