...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`prompt/cot.md`。
...
## Chain of Thought 思维链
  ...
  让模型先想后答——这一个简单的 prompt 技巧把 GPT 数学题准确率从 18% 提到 57%，进而催生了整个推理模型时代。

  
    适合阶段：Prompt 进阶
    核心机制：把"推理"显式写出来
    面试重点：CoT / ToT / GoT 演进 + 推理模型关系
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    CoT 凭什么有效？为什么"让模型多说话"能提升准确率？
    考对推理时间 compute scaling 的理解。
  

  
    "Let's think step by step" 和详细 few-shot CoT 区别在哪？
    考 zero-shot CoT vs few-shot CoT。
    ...
    ToT (Tree of Thoughts) 和 CoT 区别是什么？什么场景用 ToT？
    考进阶推理结构。
  

  
    GoT (Graph of Thoughts) 又是什么？比 ToT 强在哪？
    考前沿了解。
  

  
    推理模型（o1、R1）内置 CoT，普通模型的 CoT prompt 还有意义吗？
    考新趋势认知。
  

  
    CoT 的 hidden cost 是什么？为什么生产里要慎用？
    考工程权衡，CoT 不免费。
