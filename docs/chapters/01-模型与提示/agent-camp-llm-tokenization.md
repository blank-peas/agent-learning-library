...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`llm/tokenization.md`。
...
## Tokenization 分词
  ...
  LLM 不是按字符理解文本的，它看到的是 token——这一步决定了模型怎么算钱、怎么数数、为什么中文比英文贵。

  
    适合阶段：LLM 入门必读
    核心链路：BPE / SentencePiece / Tiktoken
    面试重点：token 与字符的区别、中英文差异
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    1 个 token 等于多少字？为什么中文和英文相差这么多？
    考基础概念 + 工程成本意识。
  

  
    为什么 GPT-4 数不清 "strawberry" 有几个 r？
    经典面试题，考你对 tokenization 本质的理解。
  

  
    BPE 是怎么工作的？跟 WordPiece、SentencePiece 区别在哪？
    考算法本身，能不能讲清训练过程。
  

  
    为什么不直接按字符 (char) 或字节 (byte) 切？
    考权衡能力，理解词表大小的 trade-off。
  

  
    同一段中文，GPT-4 和 Qwen 算出来的 token 数差好几倍。为什么？
    考对多语言 tokenizer 设计的理解。
  

  
    API 报 "context length exceeded"，但你算字符数没超。怎么解释？
    考实战经验，token != 字符。
  

  
    tokenizer 选错了会怎样？训练和推理用不同 tokenizer 会怎样？
    考生产事故意识。
