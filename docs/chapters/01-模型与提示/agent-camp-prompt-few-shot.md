...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`prompt/few-shot.md`。
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    Few-shot 为什么 work？模型不是没更新参数吗？
    考对 in-context learning 涌现机制的理解。
  

  
    0-shot、Few-shot、Fine-tune 三者的边界在哪？什么场景选哪个？
    考工程决策能力。
  

  
    示例数量从 1 增到 5 收益最大，再加效果几乎不变。为什么？
    考你跟得上 2022 后的实证发现。
  

  
    Few-shot 示例的顺序会显著影响输出，为什么？
    考对 recency bias 的认知。
  

  
    选示例时应该选"和当前输入最相似的"还是"最有代表性的"？
    考动态 few-shot vs 静态 few-shot 的差异。
  

  
    示例标签写错了（故意写反），模型表现会怎样？
    考你知不知道 Min et al. 2022 那篇反直觉的论文。
  

  
    推理模型（o1、R1）还需要 few-shot 吗？
    考新趋势，2024 后推理模型 few-shot 反而有害。
