# 模型选型与持续重评面试深挖：别再说“这个模型效果更好”

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`llm/model-selection-interview.md`。
...
## 模型选型与持续重评面试深挖：别再说“这个模型效果更好”
  ...
  面试官追问模型选型，真正想听的是你怎么证明它适合这个业务，以及什么时候该换。

  
    适合阶段：Agent 工程师 / 应用架构面
    核心能力：业务 eval · 多模型对比 · 成本延迟 · 持续重评
    脱敏原则：只保留选型方法，不保留真实项目细节
...
这些题不是在考你背不背榜单，而是在考你有没有把模型当成生产依赖来管理。
    ...
    你为什么选这个模型？评估集有多少 case，怎么分层？
    考你是不是靠体感选型，还是有可复现的业务 eval。
  

  
    通用 benchmark 分高，为什么线上业务不一定好？
    考你能不能区分 MMLU / Arena / BFCL 和业务任务。
  

  
    工具调用、意图识别、最终回复要不要用同一个模型？
    考分阶段选型和 model routing，不是全链路押一个模型。
  

  
    新模型出来了，你怎么判断要不要切？
    考持续重评、case 级 diff、灰度和回滚。
  

  
    模型 A 分数高 2 个点，但贵 3 倍，你怎么决策？
    考成本 / 延迟 / 质量的 Pareto 取舍。
  

  
    API 和自部署怎么选？数据不能出域怎么办？
    考合规、TCO、弹性扩容和运维能力。
  

  
    选错模型导致工具死循环，这句话对吗？
    考你能不能区分模型能力问题和 harness / 架构问题。
  

  
    线上已经绑定某个模型，怎么设计不被供应商锁死？
    考 provider abstraction、评估兼容、降级和双跑。
