...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`prompt/system-prompt.md`。
...
## System Prompt 设计
  ...
  System prompt 定义 Agent 的人格、规则、边界——它的设计决定产品的天花板，远比 user prompt 重要。

  
    适合阶段：Prompt 进阶
    核心要素：身份 / 任务 / 约束 / 拒答规则
    面试重点：长 system prompt 的工程化
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    System prompt 和 user prompt 在模型眼里有什么区别？
    考底层机制，能不能讲清"优先级"是怎么实现的。
  

  
    一个好的 system prompt 应该包含哪几个部分？
    考结构化设计能力。
  

  
    Anthropic / OpenAI 泄漏的 system prompt 都很长，为什么？要照搬吗？
    考你能不能分清"通用 Agent"和"特定任务"的需求差异。
  

  
    System prompt 写得越详细越好吗？什么时候反而有害？
    考权衡和实战经验。
  

  
    怎么防止用户通过 user prompt 覆盖 system prompt 的规则？
    考 prompt injection 防御意识。
  

  
    多轮对话中 system prompt 在哪个位置？会重复发吗？
    考工程实现细节。
  

  
    System prompt 怎么版本管理 + A/B test？
    考生产化能力。
