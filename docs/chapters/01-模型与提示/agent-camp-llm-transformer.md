...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`llm/transformer.md`。
...
## Transformer 架构
  ...
  用 attention 取代递归与卷积的序列建模架构，是当下所有主流 LLM 的共同底座。

  
    适合阶段：LLM 入门必读
    核心链路：Attention / Multi-Head / 位置编码 / Decoder-only
    面试重点：原理细节 + 架构演化判断
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    Transformer 取代 RNN/LSTM 凭的是什么？只是因为效果好吗？
    考你对训练并行性的理解——这是 RNN 真正死亡的原因，不是效果。
  

  
    self-attention 的 Q/K/V 各自是什么角色？为什么必须分成三个矩阵？
    考底层机制，能不能讲清楚"查询-键-值"的语义。
  

  
    attention 公式里为什么要除以 √d_k？不除会怎样？
    考数值稳定性细节，没读过论文的人答不上来。
  

  
    multi-head attention 为什么比 single-head 好？为什么不直接用一个大 head？
    考表征学习的直觉。
  

  
    位置编码为什么必要？sin/cos、可学习编码、RoPE 三种各自的动机？
    RoPE 是当前 LLM 标配，不懂会暴露知识更新滞后。
  

  
    GPT 用 decoder-only，T5 用 encoder-decoder，为什么最后是 decoder-only 赢了？
    考架构演化判断力。
  

  
    KV Cache 是什么？为什么 decoder 推理时能用、训练时不能用？
    考推理工程，连接到推理优化章节。
  

  
    attention 的 O(n²) 复杂度问题，业界有哪些缓解方案？
    考前沿了解，FlashAttention / 稀疏 attention / 线性 attention。
