...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`llm/inference-params.md`。
...
## 推理参数详解
  ...
  temperature、top_p、top_k、penalty——这些调"创造力"的旋钮，调错一档输出就走样。
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    temperature 是怎么影响输出的？数学上做了什么？
    考底层机制，不能只会背"越高越随机"。
  

  
    temperature=0 真的能保证完全确定输出吗？
    陷阱题，"理论上是"但生产里几乎做不到。
  

  
    top_p 和 top_k 区别是什么？同时设会怎样？
    考混合采样的实际效果。
  

  
    presence_penalty 和 frequency_penalty 各自惩罚什么？
    考你能不能讲清这俩的细微差别。
  

  
    RAG 场景应该用什么参数？创意写作呢？代码生成呢？
    考场景配方，区分纸上知识和实战。
  

  
    为什么有时候降 temperature 反而出现重复循环？
    考对 greedy decoding 失败模式的认知。
  

  
    seed、logprobs、logit_bias 这些参数什么时候用？
    考进阶用法和 debug 能力。
