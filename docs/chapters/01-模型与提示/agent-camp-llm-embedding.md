...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`llm/embedding.md`。
...
## Embedding 嵌入
  ...
  把离散符号映射成稠密向量——LLM 内部所有计算的起点，也是 RAG、推荐、聚类的统一数学基础。
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    token embedding、word embedding、sentence embedding 是同一个东西吗？
    考概念分层，三者完全不同。
  

  
    word2vec 时代的 embedding 和现在 LLM 内部的 embedding 有什么区别？
    考静态 vs 上下文化的本质差异。
  

  
    为什么 GPT 内部的 hidden states 不能直接当 sentence embedding 用？
    考对 decoder-only 输出语义的理解。
  

  
    选 embedding 模型时主要看什么指标？维度越高越好吗？
    考工程选型，MTEB 榜单不能盲信。
  

  
    OpenAI text-embedding-3 和 BGE 各自的特点？为什么国内常选 BGE？
    考主流模型熟悉度。
  

  
    contrastive learning 是怎么训练 embedding 的？为什么需要负样本？
    考训练原理，能不能讲清 InfoNCE。
  

  
    同一个 query 和 doc 用不同 embedding 模型编码，余弦相似度数值能不能比？
    考对向量空间不通约性的认知。
