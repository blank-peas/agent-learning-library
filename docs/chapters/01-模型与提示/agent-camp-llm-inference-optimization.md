...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`llm/inference-optimization.md`。
...
## 推理优化
  ...
  同样的模型，同样的 prompt，工程优化后的推理可以快 10 倍、便宜 10 倍——这些技术是 LLM 商业化的核心壁垒。

  
    适合阶段：进阶 + 工程化
    核心链路：KV Cache / 量化 / FlashAttention / Speculative
    面试重点：瓶颈分析 + 优化栈
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    LLM 推理的瓶颈到底是计算还是显存？为什么？
    考最基础的认知，能不能讲清 memory-bound vs compute-bound。
  

  
    Prefill 和 Decode 两个阶段的 trade-off 各是什么？
    考你能不能分清"读 prompt"和"生成 token"是两种完全不同的负载。
  

  
    KV Cache 显存怎么算？为什么长上下文会爆显存？
    考具体公式和直觉。
  

  
    FlashAttention 为什么比标准 attention 快？算法层面做了什么？
    考前沿了解，IO-aware 算法的核心思想。
  

  
    4-bit 量化为什么效果几乎不掉？INT4 和 NF4 区别？
    考量化的工程细节。
  

  
    Speculative Decoding 是什么？小模型和大模型怎么配合？
    考新兴技术，2024 后生产标配。
  

  
    vLLM 的 PagedAttention 解决什么问题？为什么传统 KV Cache 浪费严重？
    考主流推理引擎背后的核心 idea。
  

  
    continuous batching 和 static batching 区别？为什么前者吞吐高这么多？
    考批处理调度，工程实战必懂。
