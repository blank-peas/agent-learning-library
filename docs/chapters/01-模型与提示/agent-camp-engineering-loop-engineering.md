# Loop Engineering：从写 prompt 到设计驱动 Agent 的系统

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`engineering/loop-engineering.md`。
...
## Loop Engineering：从写 prompt 到设计驱动 Agent 的系统
  ...
  你的角色从「不停给 Agent 发指令的人」变成「设计那套发指令系统的人」。

  
    适合阶段：生产 / 自动化 Agent
    核心机制：调度 · 并行 worktree · 持久状态 · maker/checker
    面试重点：Loop vs Prompt / Context / Harness
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    Prompt、context、harness、loop engineering 这几层分别解决什么？为什么会演化出 loop 这一层？
    考分层认知，能不能讲清楚「人退到哪一层」。
  

  
    一个能跑一晚上的 loop 至少要具备哪几个要素？
    考可测终止条件、工具、上下文管理、失败出口、错误自适应。
  

  
    Ralph Wiggum 式的裸 while-true loop 什么时候够用、什么时候翻车？
    考可程序化验证的边界，以及 fresh context 的取舍。
  

  
    Loop engineering 和 multi-agent / orchestrator-worker 是一回事吗？
    考辨析：sub-agent 的 maker/checker 拆分 vs 通用多 Agent 协作。
  

  
    为什么并行跑多个 Agent 要用 git worktree，而不是直接开几个进程？
    考隔离、文件冲突、可回滚。
  

  
    验证（verification）为什么必须由代码或人拥有，不能交给 loop 自己？
    考 comprehension debt、cognitive surrender、maker≠checker。
  

  
    怎么估算和控制一个 loop 的成本？跑飞了怎么兜底？
    考 cadence、token 预算、重试上限、failure exit。
  

  
    怎么判断一个任务适不适合做成 loop？
    考「终止条件可测 + 进度可程序化验证」这条筛选线。
