...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`prompt/templates.md`。
...
## 提示词模板工程化
  ...
  散落在代码里的 prompt 是技术债——模板化、参数化、版本管理、评估闭环才能让 prompt 变成可维护的资产。

  
    适合阶段：生产化必读
    核心：Jinja / LangChain PromptTemplate / 版本管理
    面试重点：从手写 prompt 到 prompt as code
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    为什么不能用 Python f-string 拼 prompt？
    考你能不能讲清楚 prompt injection 和转义问题。
  

  
    Jinja2 模板比 f-string 多了什么？为什么 LLM 工程偏爱 Jinja？
    考工具选型背后的原因。
  

  
    LangChain PromptTemplate 和 ChatPromptTemplate 区别在哪？
    考主流框架的细节。
  

  
    Prompt 应该和代码一起 git 管，还是放数据库 hot reload？
    考工程权衡。
  

  
    怎么对 prompt 做 A/B test？指标怎么定？
    考产品化思维。
  

  
    Prompt 评估自动化怎么做？LLM-as-judge 的坑在哪？
    考评估方法论。
  

  
    大型 prompt（5000+ token）的拆分和组合怎么管理？
    考复杂场景工程能力。
