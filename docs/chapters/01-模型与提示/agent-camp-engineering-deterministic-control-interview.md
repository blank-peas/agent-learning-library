# Agent 确定性控制面试深挖：别把概率问题继续交给 prompt

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`engineering/deterministic-control-interview.md`。
...
## Agent 确定性控制面试深挖：别把概率问题继续交给 prompt
  ...
  模型漏段、日期算错、JSON 偶发坏掉，这些不是“再优化一下提示词”就能稳定解决的问题。

  
    适合阶段：Agent 工程师 / AI 应用架构面
    核心能力：代码下沉 · Structured Output · Verifier · 定向修复
...
这组题经常出现在项目深挖里。它不是考你会不会写 prompt，而是考你知不知道 prompt 的边界在哪里。
    ...
    一个 prompt 要求输出 A/B/C 三段，模型总漏 C，加 few-shot 还是漏，怎么办？
    考你会不会从自由文本切到结构化字段、代码校验和定向补段。
  

  
    日期窗口、金额计算、排序规则这些要不要交给模型？
    考确定性下沉：能用代码算的，不要让模型凭语义判断。
  

  
    Structured Outputs 和 JSON mode 有什么区别？为什么 JSON 合法还可能没用？
    考 schema adherence，而不是只会说“输出 JSON”。
  

  
    schema 校验通过了，为什么还可能是错的？
    考格式正确、语义正确和业务正确三层边界。
  

  
    prefill、few-shot、显式编号这些 prompt 手段还要不要用？
    考软约束和硬约束的优先级，不是非黑即白。
  

  
    模型输出坏了，是整体重试还是只修坏字段？
    考定向修复、局部重试和可解释的 fallback。
  

  
    verifier 到底验证什么？规则、模型裁判、人工 gate 怎么分工？
    考你能不能把验证拆成格式、范围、权限、一致性和风险。
  

  
    这类稳定性问题怎么进评估集？
    考 badcase 归因、required fields、case-level diff 和回归门禁。
