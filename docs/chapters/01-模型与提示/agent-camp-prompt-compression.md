...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`prompt/compression.md`。
...
## Prompt Compression 提示词压缩
  ...
  用 LLMLingua、token pruning 等技术把长 prompt 压到 1/3-1/5——省钱、降延迟、不掉效果，长上下文时代必备。

  
    适合阶段：成本/延迟优化
    核心：LLMLingua / 摘要 / KV Cache 复用
    面试重点：压缩 vs 长上下文模型的权衡
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    为什么需要 prompt compression？长上下文模型不就解决问题了吗？
    考你能不能讲清"塞得下"和"用得好"是两回事。
  

  
    LLMLingua 是怎么工作的？为什么用小模型就能给大模型压缩？
    考压缩算法的底层机制。
    ...
    除了 LLMLingua，还有哪些压缩方法？各自适合什么场景？
    考方法论广度。
  

  
    什么场景下不该压缩？压缩的常见 failure mode 是什么？
    考边界认知。
  

  
    Prompt compression 和 KV Cache、prefix caching 是替代关系还是互补？
    考工程权衡。
