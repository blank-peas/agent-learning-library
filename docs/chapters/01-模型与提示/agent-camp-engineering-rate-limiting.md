...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`engineering/rate-limiting.md`。
...
## LLM Agent 的限流、降级与熔断
  ...
  OpenAI Tier 1 账号 RPM 500、TPM 30K，并发处理 1000 个用户分分钟雪崩——限流不只是 429 退避这么简单，它是 Agent 能不能从 demo 走到生产的分水岭。

  
    适合阶段：生产 / 容量规划
    核心：token bucket + backoff + breaker + fallback
    面试重点：tier 模型 + 雪崩防护 + 多账号策略
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    OpenAI / Anthropic 的 tier 系统到底限的是什么？RPM、TPM、ITPM、OTPM 分别是什么意思？
    考对厂商真实限额结构的理解——不是泛泛说"有限流"，而是知道哪个维度先打爆。
  

  
    为什么固定间隔重试在 LLM 应用里会引发雷暴？jitter 的数学逻辑是什么？
    考分布式系统直觉——多个客户端同步重试的灾难性后果。
  

  
    遇到 429 你怎么处理？为什么不能直接看 Retry-After 等?
    考对 token 级限流的理解——Retry-After 只是建议，本地令牌桶才是根本。
  

  
    Circuit breaker 在 LLM 调用里的阈值怎么定？和传统微服务有什么不一样？
    考量化思维——单次调用贵 + 延迟高，熔断要比传统服务更激进。
  

  
    主模型挂了切备用模型，怎么保证体验一致？哪些是必须同步切换的？
    考 fallback chain 的实战细节——不是换个 endpoint 就完事。
  

  
    一个用户 abuse 把整个公司的 OpenAI 配额烧光，你怎么防?
    考多租户限流的设计——per-user quota + 全局水位双层保护。
  

  
    多 API key 之间怎么负载均衡？key 之间也要做熔断吗？
    考真实生产架构——大公司都有 key pool，但 key 级故障隔离常被忽略。
  

  
    限流和降级的关系？什么场景该降级、降到哪？
    考产品 + 工程结合——三层降级（模型 / 功能 / 体验）的边界。
