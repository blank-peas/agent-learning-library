...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`agent/react-pattern.md`。
...
## ReAct 模式 Reasoning + Acting
  ...
  ReAct 让模型在推理、行动、观察之间往返，用工具反馈修正下一步。

  
    适合阶段：工具调用 Agent
    核心机制：Thought / Action / Observation
    面试重点：何时循环，何时停止
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    Thought 能不能暴露给用户？现代产品里怎么处理？
    考 chain-of-thought 安全和可观测性取舍。
  

  
    ReAct 为什么容易循环？怎么加停止条件？
    考 max steps、重复动作检测、证据评分。
  

  
    Observation 应该返回多长？返回原文还是结构化结果？
    考上下文预算和工具契约。
  

  
    ReAct 适合长任务吗？什么时候换 Plan-and-Execute？
    考短视决策和长任务规划差异。
    ...
    如何评估 ReAct Agent？
    考轨迹评估：action、args、observation、final answer。
