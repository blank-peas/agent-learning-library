...
> **资料来源**：[Agent Interview 100](https://github.com/BigKunLun/Agent-Interview-100) · 原文件：`06-planning-and-reasoning/049-cot-and-tot.md`。
...
```mermaid
flowchart TD
    subgraph COT["CoT —— 线性链（一条路走到底，不回头）"]
        P["问题"] --> S1["步骤 1"] --> S2["步骤 2"] --> ANS["答案"]
    end
    subgraph TOT["ToT —— 探索树（生成·评估·剪枝）"]
        TQ["问题"] --> TA["思路 A<br/>评估 0.8 · 继续探索"]
        TQ --> TB["思路 B<br/>评估 0.5 · 继续探索"]
        TQ --> TC["思路 C<br/>评估 0.1 · 剪枝"]
        TA --> A1N["A1 · 评估 0.9<br/>★ 最优"]
        TA --> A2N["A2 · 评估 0.3<br/>剪枝"]
        TB --> B1N["B1 · 评估 0.2<br/>剪枝"]
    end
    classDef proc fill:#e3f2fd,stroke:#1565c0,color:#0d47a1
    classDef err fill:#ffebee,stroke:#c62828,color:#b71c1c
    class P,S1,S2,ANS,TA,TB,A1N proc
    class TC,A2N,B1N err
```
*CoT 一条链走到底不回头，ToT 每步生成分支、评估打分、剪枝回溯。*
