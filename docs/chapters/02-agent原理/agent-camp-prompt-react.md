...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`prompt/react.md`。
...
## ReAct Prompt 模式
  ...
  Thought / Action / Observation 三段式，让模型从"会答题"变成"会做事"——Agent 时代的奠基 prompt 范式。

  
    适合阶段：Prompt 进阶 → Agent 入门
    核心机制：交替推理与行动
    面试重点：和原生 function calling 的区别
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    ReAct 三段式 Thought/Action/Observation 各自是什么？为什么要 Thought 这一步？
    考你能不能讲清"为什么不直接 Action"。
  

  
    ReAct 和"直接 function calling"的区别在哪？
    考你对 OpenAI tool use 演化的理解。
  

  
    ReAct 论文里实测的提升幅度有多大？哪些任务上效果显著？
    考具体数据，能不能讲出 HotpotQA、ALFWorld 等基准。
  

  
    ReAct 失效的典型模式是什么？怎么 debug？
    考实战经验，区分纸上知识。
  

  
    ReAct 的 Thought 如果是幻觉怎么办？怎么约束？
    考工程实战，幻觉的处理。
  

  
    2024 后原生 function calling 这么强，ReAct 还有意义吗？
    考新趋势，ReAct 的演化和价值。
  

  
    ReAct 和 CoT 是什么关系？ReAct = CoT + 工具？
    考概念辨析。
...
def run_react(question, llm, max_steps=10):
    ...
    for step in range(max_steps):
        ...
        if tool_name not in TOOLS:
            ...
            obs = TOOLS[tool_name](https://github.com/yibo365/agent-camp)
