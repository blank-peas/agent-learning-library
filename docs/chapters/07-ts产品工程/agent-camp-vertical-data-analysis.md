...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`vertical/data-analysis.md`。
...
## 数据分析 Agent
  ...
  2023 年 7 月 ChatGPT 推出 Code Interpreter（后改名 Advanced Data Analysis），让"用 LLM 做数据分析"从概念变成产品。这一类 Agent 的特殊之处在于——它不是"调一堆数据工具"，而是"自己写 Python 代码、在沙箱里跑、看结果决定下一步"。本文讲清楚这种 Code Interpreter 范式为什么比 Text-to-SQL 通用，企业级数据分析 Agent 怎么对接真实仓库，以及它和 BI 工具（Tableau/PowerBI）的边界。

  
    适合阶段：进阶 / 数据团队
    核心链路：Understand → Code → Execute → Visualize → Iterate
    面试重点：Code Interpreter 沙箱 + Text-to-SQL 局限 + 数据安全边界
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    数据分析 Agent 的核心范式是 Code Interpreter，为什么不是 Text-to-SQL？
    考范式差异——Text-to-SQL 只能查数据库且语义贫弱；Code Interpreter 能做清洗、统计、建模、可视化全流程，且能跨多种数据源。
  

  
    Code Interpreter 在沙箱里跑代码，沙箱怎么设计？为什么必须沙箱？
    考安全意识——LLM 可能生成危险代码（rm、网络请求、读敏感文件），沙箱用 Docker / WASM 隔离 + 资源限制 + 网络白名单。
  

  
    Code Interpreter 的中间结果（图表、表格）怎么反馈给 LLM？纯文本就够吗？
    考多模态反馈——表格转成 markdown 喂给 LLM、图表渲染成图片用 vision model 看、错误消息原样回传。
  

  
    Text-to-SQL 在生产里成功率多少？最常见的失败模式是什么？
    考真实瓶颈——SOTA 在 Spider 上 ~85%，BIRD 上 ~70%，但真实企业仓库通常 < 50%，主要卡在 schema 复杂、隐式业务语义、多表 join。
  

  
    数据分析 Agent 怎么对接企业内部的 PostgreSQL/Snowflake/BigQuery？直接给数据库密码？
    考安全架构——绝对不能给数据库直连，要走中间层（API、view、row-level security），加查询审计 + 限速。
  

  
    数据分析 Agent 和 BI 工具（Tableau / PowerBI / Looker）什么关系？替代还是补充？
    考产品定位——BI 工具是"做 dashboard 给所有人看"，Agent 是"做 ad-hoc 一次性分析"，互补不替代。
  

  
    数据分析 Agent 的"幻觉"最危险——它可能编出看似合理但错的数字。怎么防？
    考数据可信度保障——强制让 Agent 引用数据查询的 raw output、用户必须能看到生成的代码、关键数字加来源标注。
  

  
    企业落地数据分析 Agent，最大的非技术障碍是什么？
    考产品 + 组织判断——不是技术问题，是数据治理（schema 文档化、metric 定义对齐）和组织信任（业务方接受 AI 给的数字）。
