...
> **资料来源**：[Agent Interview 100](https://github.com/BigKunLun/Agent-Interview-100) · 原文件：`09-safety-and-alignment/084-agent-alignment.md`。
...
| 维度 | LLM 对齐 | Agent 对齐 |
|------|---------|-----------|
| 范围 | 控制文本输出的质量和安全性 | 控制自主决策、工具使用、多步行动 |
| 失败后果 | 生成不当内容（可以过滤） | 执行有害操作（删除数据、发送信息、金融交易） |
| 监督 | 每次输出都可以检查 | 多步执行中间步骤难以逐一审查 |
| 可逆性 | 文本输出不直接改变世界 | 许多操作不可逆 |
| 额外挑战 | — | 规格游戏：找到“合规但有害”的捷径；目标漂移：长期执行中偏离原始目标；能力跳跃：能力提升后发现新的“作弊”方式；多 Agent 涌现：个体对齐不保证集体对齐 |
