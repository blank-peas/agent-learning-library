...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`context/compression.md`。
...
## 上下文压缩与摘要
  ...
  摘要、抽取、分层压缩——上下文工程的核心动作。把 100K 上下文压到 10K 还保留 90% 有效信息，是长 Agent 必备能力。

  
    适合阶段：上下文工程进阶
    核心：摘要 / 抽取 / 分层 / 滚动
    面试重点：和 prompt-level 压缩的区别
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    上下文压缩和 prompt compression 是同一回事吗？
    考概念分层，前者偏架构后者偏算法。
  

  
    Map-Reduce、Refine、Stuff 三种摘要策略各自适合什么场景？
    考 LangChain 经典三策略。
  

  
    多轮对话历史用滚动摘要 (rolling summary)，关键信息怎么不丢？
    考工程细节，能不能讲清"分层记忆"思路。
  

  
    抽取式摘要 vs 生成式摘要在 Agent 场景哪个更合适？
    考权衡，事实保真度 vs 信息密度。
  

  
    压缩是用 LLM 还是用 embedding？两种各自的代价？
    考成本意识。
  

  
    压缩后怎么验证信息没丢？怎么 debug？
    考评估方法论。
  

  
    Claude Code 用什么策略管理长 session 的上下文？
    考实战观察，了解主流 Agent 怎么做。
