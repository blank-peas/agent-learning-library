...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`workflow/workflow-vs-agent.md`。
...
## 工作流与 Agent 的边界
  ...
  Anthropic 2024 年底的《Building Effective Agents》把 LLM 系统分成两大类：Workflow（预定义路径编排 LLM）和 Agent（模型自主决定路径和工具）。90% 的"Agent"项目其实该用 Workflow——这篇讲清楚怎么判断、怎么选、以及两者怎么混用。

  
    适合阶段：架构选型 / 面试高频
    核心：控制流归属——代码写死 vs 模型动态决定
    面试重点：边界判断 + Anthropic 分类法
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    Anthropic 怎么定义 Workflow 和 Agent？本质区别是什么？
    考能不能一句话讲出"控制流归属"——代码写死 vs 模型运行时决定。
  

  
    你的项目里应该用 Workflow 还是 Agent？怎么决策？
    考工程判断力，面试官最怕听到"上 Agent 更灵活"这种无脑回答。
  

  
    Anthropic 提出的五种 Workflow 模式分别解决什么问题？
    考对 prompt chaining / routing / parallelization / orchestrator-workers / evaluator-optimizer 的理解。
  

  
    Agent 在生产中最常见的失败模式是什么？
    考对 compounding errors、context overflow、tool confusion、unbounded cost 的认知。
  

  
    大部分生产系统是纯 Workflow 还是纯 Agent？
    考能不能答出"混合架构"——确定性路由 + 局部自主决策。
  

  
    Workflow 的维护成本在什么节点会超过 Agent？
    考对"分支爆炸"的理解——分支超 15 个就该考虑让模型动态路由。
  

  
    怎么在 Agent 里保留 Workflow 的可审计性？
    考 structured output + guardrails + trace logging 的工程组合。
