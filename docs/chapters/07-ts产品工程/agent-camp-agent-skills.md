# Agent Skills：把经验做成可加载能力

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`agent/skills.md`。
...
## Agent Skills：把经验做成可加载能力
  ...
  Skill 不是一个新工具，而是让 Agent 按需加载任务经验的工程单元。

  
    适合阶段：Agent 进阶 / 团队工程化
    核心机制：Progressive Disclosure
    面试重点：Skill / Tool / MCP / Subagent 边界
...
### 面试官想考什么
    ...
    Skill 和 Tool 到底差在哪？为什么不能都写成 function calling？
    考你能否区分流程知识和执行接口。
  

  
    为什么 Skill 要按需加载，而不是全部塞进 system prompt？
    考上下文成本、位置偏置和上下文污染。
  

  
    一个好的 SKILL.md 应该写什么，不该写什么？
    考面向模型的文档设计，而不是给人写 README。
  

  
    Skill 的 description 为什么比正文还关键？
    考触发路由。description 写烂，正文再好也不会被加载。
  

  
    Skill、MCP Server、Subagent、Memory 怎么组合？
    考系统分层：知识、工具、执行者、状态各归各位。
  

  
    Skill 会不会变成新的 prompt injection 入口？怎么防？
    考供应链、权限和工作区信任。
...
- Compatibility checklist: [checklist.md](https://github.com/yibo365/agent-camp)
- Example report: [examples/risky-change.md](https://github.com/yibo365/agent-camp)
```
