...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`tools/sandbox.md`。
...
## 工具沙箱与权限
  ...
  LLM Agent 一旦被 prompt injection 或自身错误诱导，调用 rm、send_email、execute_code 的后果是不可逆的——沙箱和最小权限是把"软规则"变成"硬边界"的最后一道防线。

  
    适合阶段：生产化必读
    核心：隔离 + 最小权限 + 人工确认
    面试重点：能不能讲清"为什么 prompt 防御不够"
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    为什么 Agent 工具调用必须放沙箱，而传统后端的工具函数不用？
    考你能不能讲清 LLM 的"不可信指令源"特性。
  

  
    Docker / Firecracker / gVisor / WebAssembly 四种隔离技术怎么选？
    考你对隔离强度 × 启动开销 × 兼容性的权衡。
  

  
    哪些 Agent 操作必须 human-in-the-loop？判据是什么？
    考"不可逆 × 副作用大"的工程判断。
  

  
    为什么 path traversal 在 LLM Agent 场景比传统 Web 更危险？
    考你对"自然语言路径 + 自动执行"组合的理解。
  

  
    Claude Code / Cursor 的代码执行权限怎么设计？
    考你看过没有，知不知道默认 ask、配置 allowlist 的工程模式。
  

  
    如何防止 LLM 把 secrets 写入沙箱内文件后被外泄？
    考你对"沙箱内合法 → 出网通道非法"链路的全局视角。
  

  
    MCP server 跑在本地，权限边界应该怎么设计？
    考你对新协议下"工具供应链"风险的认知。
  

  
    用户确认机制能被 jailbreak 绕过吗？怎么防？
    考你知不知道"UI 信任边界"必须在 Agent 之外。
