...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`context/caching.md`。
...
## 上下文缓存
  ...
  同样的 system prompt 喂 100 次，不开 cache 你付 100 次 prefill 钱——四家 API 的 prompt cache 是长上下文应用唯一能 scale 的杠杆。

  
    适合阶段：生产 / 成本治理
    核心：prefix 命中策略 + 四家机制对比
    面试重点：cache 与 KV Cache 的区别、命中率设计
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    API 层的 prompt cache 和推理引擎里的 prefix caching 是不是一回事？
    考你能不能分清"产品功能"和"底层优化"。
  

  
    Anthropic、Gemini、DeepSeek、OpenAI 四家的 cache 机制有什么本质区别？
    考四家 API 的细节掌握度。
  

  
    为什么 cache 必须 prefix 匹配？为什么换个标点就可能全 miss？
    考对 KV Cache 物理本质的理解。
  

  
    Anthropic 写入贵 25%、命中省 90%——什么时候开 cache 反而亏？
    考 break-even 测算能力。
  

  
    Agent 多轮对话场景下，怎么布局 prompt 让 cache 命中率最高？
    考工程实战，能不能讲清固定段与变化段的分层。
  

  
    为什么 cache hit rate 上线后会悄悄下降？怎么监控？
    考可观测性，cache miss 不报警是常见线上事故。
  

  
    OpenAI 的自动 cache 跟 Anthropic 的显式 cache_control 各有什么优劣？
    考 API 设计哲学的辨析。
