...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`workflow/llamaindex-workflows.md`。
...
## LlamaIndex Workflows
  ...
  LlamaIndex 团队发现 DAG（有向无环图）在 Agent 场景下撞墙——无法表达循环和自修正。于是他们换了个思路：不画图，用事件驱动。每个步骤监听特定事件、处理后发射新事件——控制流由事件的流转隐式定义，而不是由显式的边连接。

  
    适合阶段：了解编排范式多样性
    核心：Step + Event + Context
    面试重点：事件驱动 vs 图编排的权衡
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    LlamaIndex Workflows 的核心概念是什么？和 LangGraph 有什么本质区别？
    考能不能讲清"事件驱动 vs 图编排"的范式差异。
  

  
    LlamaIndex 为什么从 Query Pipeline（DAG）转向事件驱动的 Workflows？
    考对 DAG 局限性的理解——DAG 不能表达循环，Agent 场景需要循环。
  

  
    事件驱动架构在 LLM 编排里的优势和劣势分别是什么？
    考对范式 trade-off 的理解——灵活但控制流不可视。
  

  
    什么场景选 LlamaIndex Workflows，什么场景选 LangGraph？
    考选型判断——不是"哪个好"，而是各自的 sweet spot。
  

  
    Workflows 的 Context 和 LangGraph 的 State 有什么区别？
    考对状态管理机制差异的理解。
