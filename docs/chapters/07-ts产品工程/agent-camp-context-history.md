...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`context/history.md`。
...
## 会话历史管理
  ...
  每轮对话都要把完整 messages 重发给 LLM——history 单调增长是所有多轮 Agent 的共同宿命。本文讲在一个 session 里怎么管这堆消息：留什么、压什么、扔什么、怎么不踩 cache。

  
    适合阶段：Agent 进阶
    核心：in-session 的 messages 管理
    面试重点：策略选型 + cache 配合
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    为什么每轮对话都要把整个 messages 数组重发给 LLM？这意味着什么？
    考对 LLM 无状态本质的理解，能不能讲清 prompt 单调增长的根因。
  

  
    ConversationBufferWindowMemory、SummaryMemory、SummaryBufferMemory 各自什么场景？
    考 LangChain 经典三件套的取舍辨析。
  

  
    滑动窗口截断最容易丢的是什么信息？怎么补救？
    考工程细节，能不能说出"system 之后的早期约束"被忽略的真实坑。
  

  
    什么时候触发摘要压缩？基于 token、轮次还是主题切换？
    考触发策略设计，是否考虑过 cache 命中率。
  

  
    history 怎么布局才能让 prefix cache 不失效？
    考成本意识——是否理解 cache 命中和 history 动态修改的矛盾。
  

  
    从 history 里召回相关片段——什么时候这么做有用？
    考向量召回作为 history 管理手段的边界。
  

  
    Claude Code 的 auto-compact 触发条件是什么？压缩后保留什么？
    考实战观察，了解主流编程 Agent 的真实做法。
  

  
    history 摘要时把订单号 ABC-1234 写成 ABC-1243 怎么防？
    考关键事实保真的工程方案。
