...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`agent/definition.md`。
...
## Agent 定义与认知架构
  ...
  Agent 是围绕目标、状态、工具和反馈推进任务的系统。

  
    适合阶段：Agent 入门与系统设计
    核心机制：Goal / State / Tools / Feedback
    面试重点：Agent vs Workflow
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    什么才算 Agent？和普通 Chatbot、RAG、Workflow 的边界在哪里？
    考你能否把“会调用工具”和“能自主推进任务”分开。
  

  
    Agent 的最小运行单元包含哪些组件？
    考 goal、state、tool registry、policy、feedback、trace。
  

  
    为什么 Anthropic 会区分 workflow 和 agent？
    考控制流由代码写死，还是由模型动态决定。
  

  
    Agent 的“自主性”应该怎么限制？
    考权限、预算、最大步数、人工确认、拒绝策略。
  

  
    一个 Agent 失败时，你怎么判断是模型、工具、状态还是策略的问题？
    考工程排障能力，不接受一句“模型不够强”。
  

  
    Agent 架构里的 memory、planning、reflection 分别解决什么问题？
    考认知模块的职责边界。
  

  
    面试里让你设计一个客服 Agent，你会先做 workflow 还是 agent？
    考工程克制和风险意识。
...
def run_agent(order_id: str, max_steps: int = 6) -> State:
    ...
    for _ in range(max_steps):
        ...
        if action == "finish":
            ...
            return state
        obs = TOOLS[action](https://github.com/yibo365/agent-camp)
