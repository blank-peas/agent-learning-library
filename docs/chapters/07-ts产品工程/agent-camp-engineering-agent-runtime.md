# 从业务 Agent 到 Agent Runtime

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`engineering/agent-runtime.md`。
...
## 从业务 Agent 到 Agent Runtime
  ...
  模型提出候选动作，runtime 决定能不能执行、怎么执行、何时中断、如何记录和评估。

  
    适合阶段：系统设计 / 生产落地
    核心机制：Control + Tools + Memory + Context + Safety + Eval
    面试重点：业务 Agent 如何抽象成平台 Runtime
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    为什么生产级 Agent 不能只是 Chatbot + tools？Runtime 多出来的职责是什么？
    考你是否能把模型能力和工程控制面分开。
  

  
    Workflow、受限 Agent Loop、Agentic Runtime 三种形态怎么选？
    考你能不能按业务风险和自主性做架构分层。
  

  
    一个 Agent Runtime 至少要有哪些模块？哪些模块可以先不做？
    考 MVP 到平台化的演进判断。
  

  
    为什么要让 Orchestrator 主控循环，而不是让 LLM 自己决定一直跑？
    考状态机、终止条件、权限校验和 fallback。
  

  
    工具白名单为什么应该动态计算，而不是在 prompt 里写一串工具？
    考 intent、state、permission、risk level 四个约束维度。
  

  
    长期记忆为什么不能让模型自由写？Memory write path 应该怎么设计？
    考候选记忆、证据、冲突检测、TTL 和人工确认。
  

  
    强约束业务场景里，模型应该负责什么，确定性系统应该负责什么？
    考金融、政务、企业服务里的责任边界。
  

  
    怎么判断一个业务 Agent 已经具备平台化 Runtime 的雏形？
    考抽象能力：能不能从单场景经验上升到通用运行时。
