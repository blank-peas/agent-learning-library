...
> **资料来源**：[Agent Interview 100](https://github.com/BigKunLun/Agent-Interview-100) · 原文件：`10-production-and-deployment/094-scaling-strategies.md`。
...
| --- | --- | --- |
| 请求处理时间 | 10-100ms | 1-60s（甚至更长） |
| 资源消耗 | 可预测 | Token 消耗不确定（变化 10x） |
| 状态 | 无状态（通常） | 多步执行有状态 |
| 扩展指标 | CPU/内存 | 并发数/队列深度 |
| 失败模式 | 超时/500 | API 限流/成本爆炸 |
| 连接 | 短连接 | 长连接/SSE 流式 |
...
flowchart LR
    A["用户请求"] --> B["API 网关"]
    B --> Q["消息队列<br/>队列深度驱动扩缩容"]
    Q --> W["无状态 Worker 池<br/>状态全部外置"]
    W --> G["LLM 网关<br/>多提供商分流 + 故障转移"]
    W --> S["状态存储<br/>Redis / PG"]
    W --> C["语义缓存<br/>减少 LLM 调用"]
    W --> R["流式响应<br/>SSE / WebSocket"]
    G --> R
    classDef proc fill:#e3f2fd,stroke:#1565c0,color:#0d47a1
    classDef store fill:#e8f5e9,stroke:#2e7d32,color:#1b5e20
    classDef neutral fill:#eceff1,stroke:#546e7a,color:#37474f
    class A,B,Q,W,R proc
    class S,C store
    class G neutral
```
*高并发公式：无状态 Worker + 队列解耦 + 外部状态，扩容看队列深度不看 CPU——真瓶颈在上游 API 限流。*
