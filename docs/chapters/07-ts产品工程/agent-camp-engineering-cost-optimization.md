...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`engineering/cost-optimization.md`。
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    一个高频 RAG Agent 月账单 50 万，你按什么顺序优化？
    考成本结构分析——能不能先拆账单再动手，而不是无脑套技术。
  

  
    Model routing 怎么判断请求"难度"？路由器本身的成本怎么 cover？
    考路由实现的工程细节——路由错一次的代价 vs 省下的钱。
  

  
    Batch API 50% 折扣，什么场景能用、什么不能？
    考"延迟换价格"的边界——异步场景的识别能力。
  

  
    Semantic cache 的 hit rate 怎么提高？什么场景必然失效？
    考向量相似度阈值、长尾 query、上下文依赖的辨析。
  

  
    用小模型省钱和保证质量的边界在哪？怎么测量这条线？
    考能不能拿出量化方法——不是凭感觉说"小模型够用"。
  

  
    output token 失控是怎么发生的？怎么挡？
    考对 output 单价（通常是 input 的 3-5 倍）的敏感度。
  

  
    为什么"用更便宜的模型"经常反而更贵？
    考端到端思考——重试次数、上下文膨胀、人工兜底成本。
