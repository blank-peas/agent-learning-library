...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`frameworks/pi.md`。
...
## Pi 深度剖析
  ...
  Claude Code 内置了 Plan Mode、子 Agent、权限弹窗、MCP、后台 Bash——Pi 的做法正好相反：内核什么都不内置，全部通过 Extension 实现。它把自己定义为"最小终端编程 harness"，5.9 万 star，225+ releases，核心理念是"primitive 优先于 feature"。

  
    适合阶段：进阶 / 生产
    核心链路：Minimal Core → Extension → Skill → Prompt Template
    面试重点：极简内核哲学 + Extension 架构 + Session 树
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    Pi 说自己是"最小内核"，它的内核到底包含什么、不包含什么？
    考架构取舍的理解——内核只有 LLM 对话循环 + 工具调用 + 状态管理，MCP/子 Agent/权限/Plan Mode 全部外置。
  

  
    Pi 的 Extension 和 MCP Server 有什么区别？为什么 Pi 不内置 MCP？
    考工具协议分层——Extension 是进程内的 TypeScript 模块，能访问 TUI、键盘快捷键、事件系统；MCP 是进程间协议。两者解决的问题不同。
  

  
    Pi 的四种运行模式（Interactive/Print/RPC/SDK）分别适合什么场景？
    考对 Agent 消费方式的理解——不是所有场景都需要 TUI，脚本化、管道化、嵌入式各有需求。
  

  
    Pi 的 Session 是"树状"结构，和线性对话历史有什么不同？有什么好处？
    考状态管理设计——树结构允许分支探索、回溯、并行实验，线性历史做不到。
  

  
    Pi 的供应链安全措施（exact pinning、shrinkwrap、2-day 发布延迟）为什么重要？
    考安全意识——编程 Agent 能执行任意代码，它自身的依赖链被污染等于 RCE（远程代码执行）。
  

  
    Pi 和 Claude Code 的核心架构差异在哪？
    考两种设计哲学的对比——Claude Code 是"全功能内置，开箱即用"；Pi 是"最小内核，自己组装"。不同用户群、不同 trade-off。
  

  
    "用户在 Agent 工作时可以发送消息打断它"——这在工程上怎么实现？
    考并发模型的理解——中断当前工具执行 vs 排队等待，两种策略的 trade-off。
