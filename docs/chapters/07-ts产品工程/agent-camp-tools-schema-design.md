...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`tools/schema-design.md`。
...
## 工具 Schema 设计
  ...
  模型看不到你的代码——它只能看到 schema 里的名字、描述和参数说明。这层薄薄的文本就是它做决定的全部信息。本文讲怎么把这一层写好。

  
    适合阶段：Agent 工程化
    核心：name / description / params / return 的设计原则
    面试重点：诊断"模型调错"是 schema 还是模型问题
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    模型为什么会调错工具？是模型问题还是 schema 问题，怎么 debug？
    考核心：能不能区分"指令理解错"和"工具描述写得让模型必然误解"。
  

  
    tool description 应该用第一人称（"Use this when..."）还是第三人称（"This function returns..."）？为什么？
    考有没有真的读过 OpenAI / Anthropic 的官方建议，理解模型的视角。
  

  
    参数用 string + 自由文本还是 enum？什么时候用哪个？
    考类型设计的取舍——表达力 vs 约束。
  

  
    一个工具的 description 多长合适？什么时候该拆成两个工具？
    考粒度选择的判据。
  

  
    返回值用结构化 JSON 还是自然语言文本？
    考工程经验——大多数人这里都是直觉答错。
  

  
    schema 改了能热更新吗？已经跑着的对话怎么办？
    考生产意识，是否考虑过 cache、对话连贯性。
  

  
    OpenAI 和 Anthropic 在 tool schema 上有哪些不兼容的差异？
    考跨厂商工程经验：oneOf / additionalProperties / description 长度限制。
  

  
    给你 20 个工具的 schema，让模型选——你怎么排序、怎么裁剪？
    考工具集规模化时的实际问题（选择空间爆炸）。
