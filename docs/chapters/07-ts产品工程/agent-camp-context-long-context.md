...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`context/long-context.md`。
...
## 长上下文模型对比
  ...
  Claude 200K、Gemini 2M、Qwen 1M、Kimi 2M——长上下文之争背后的技术路线、实测能力、应用边界。
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    模型从训练时 4K 扩展到 200K 上下文，技术上做了什么？
    考对 RoPE 外推 / YaRN / 训练阶段的理解。
  

  
    "100% Needle in a Haystack" 这个营销说法到底意味着什么？
    考评估方法论。
  

  
    Gemini 2M、Claude 200K、Qwen 1M 各家在长上下文上的技术路线区别？
    考主流模型差异。
  

  
    长上下文模型的成本结构是怎样的？为什么不是简单的"长度 ×单价"？
    考定价细节，prefill / cache 等。
  

  
    什么场景下长上下文真的有优势？什么场景下 RAG 仍然更好？
    考工程决策。
  

  
    同时支持长上下文 + 推理（如 o1、R1 Thinking）会有什么新挑战？
    考前沿趋势。
  

  
    KV Cache 显存会随上下文长度爆炸吗？怎么应对？
    考工程实现细节。
