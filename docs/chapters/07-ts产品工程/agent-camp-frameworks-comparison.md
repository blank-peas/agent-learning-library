...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`frameworks/comparison.md`。
...
## Agent 框架选型决策树
  ...
  每个框架的 README 都说"我什么都能做"——选型最重要的不是看每家有什么功能，而是看在你的场景里哪些功能你真用得上、哪些是负担。本文用"用户问题倒推框架选择"的方式给出可执行的决策路径。

  
    适合阶段：进阶 / 决策
    核心问题：场景 → 约束 → 框架
    面试重点：选型理由 + trade-off 阐述
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    给一个具体业务场景（如"做内部知识库问答"），你的框架选型怎么推？
    考工程判断——能不能从"用户需求 → 技术约束 → 框架选择"推导，而不是直接背答案。
  

  
    什么场景下你会选择不用任何框架，直接用 LLM 厂商 SDK 写循环？
    考反模式意识——框架不是万能的，简单场景过度工程化是常见错误。
  

  
    LangChain 被骂"抽象太厚"，但仍是市占率最高的框架，怎么解释？
    考行业认知——抽象厚 ≠ 没用，对于团队协作和生态整合，统一抽象有不可替代的价值。
  

  
    Claude Agent SDK 和 OpenAI Agents SDK，技术选型时怎么决定？
    考多维权衡——不只是"模型偏好"，还有团队既有 API key、合规要求、工具生态、cost。
  

  
    多 Agent 协作场景，LangGraph、OpenAI Agents、Hermes Agent 各自的边界？
    考对编排范式的理解——图状态机、Handoff、自主学习是三种不同思路。
  

  
    个人/团队用的 AI 助手，你会选 OpenClaw 还是 Hermes Agent？
    考产品定位辨析——OpenClaw 偏渠道整合，Hermes 偏自主学习，目标用户不同。
  

  
    选型后发现框架不合适，迁移成本怎么估？怎么降低锁定风险？
    考长期工程意识——锁定风险评估、抽象隔离、迁移路径预留。
