...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`agent/agent-loop.md`。
...
## Agent 运行循环 Agent Loop
  ...
  一个可靠 Agent 的循环，必须能计划、行动、观察、更新状态，并且知道什么时候停。

  
    适合阶段：Agent 工程实现
    核心机制：Think / Act / Observe / Reduce
    面试重点：状态机与停止条件
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    一个 Agent loop 最少包含哪些步骤？
    考 plan / action / observation / state update / stop。
  

  
    为什么不能用 while true 一直让模型决定下一步？
    考循环失控、重复调用、成本和安全。
  

  
    observation 应该怎么写回 state？谁来负责写回？
    考 reducer，不能让模型自称状态已更新。
  

  
    Agent loop 里的错误怎么分类处理？
    考 retryable、non-retryable、permission、validation。
  

  
    怎么设计 stop condition？
    考 max steps、目标达成、证据不足、人工交接。
  

  
    ReAct loop 和事件驱动状态机有什么关系？
    考论文模式到工程状态机的转换。
  

  
    如何评估一次 loop 是否跑得好？
    考 trace 指标、无效调用、成功率、延迟、成本。
