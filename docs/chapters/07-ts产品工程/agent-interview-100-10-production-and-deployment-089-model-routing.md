...
> **资料来源**：[Agent Interview 100](https://github.com/BigKunLun/Agent-Interview-100) · 原文件：`10-production-and-deployment/089-model-routing.md`。
...
flowchart TD
    A["用户请求"] --> B{"复杂度评估"}
    B -- "简单" --> C["小模型<br/>gpt-4o-mini"]
    B -- "中等" --> D["中端模型<br/>claude-sonnet"]
    B -- "复杂" --> E["强模型<br/>gpt-4o / o3"]
    E --> F["响应"]
    D --> F
    C -- "置信度 ≥ 0.8" --> F
    C -- "置信度 < 0.8<br/>级联升级" --> E
    ...
    class A,C,D,E,F proc
    class B decision
```
*路由让每个请求用"刚好够用"的模型：26% 强模型调用保住 95% 质量、砍 85% 成本；级联变体则先试小模型，置信度不足再升级。*
