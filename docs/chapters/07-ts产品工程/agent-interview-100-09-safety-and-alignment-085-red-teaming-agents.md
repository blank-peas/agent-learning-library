...
> **资料来源**：[Agent Interview 100](https://github.com/BigKunLun/Agent-Interview-100) · 原文件：`09-safety-and-alignment/085-red-teaming-agents.md`。
...
```mermaid
flowchart TD
    TM["威胁建模<br/>MITRE ATLAS"] --> AP["攻击计划"]
    AP --> AU["自动化攻击<br/>PyRIT / Garak"]
    AP --> MU["手动攻击<br/>多步操纵 / 社工"]
    AU --> RP["报告与修复"]
    MU --> RP
    RP -- "定期重测（季度）" --> TM
    ...
    class AU,MU err
```
*红队是流程不是事件：先建模后攻击，自动化打已知、人工探未知，按季度循环重测。*
...
| 成熟度等级 | 实践 |
| --- | --- |
| Level 1: 临时性 | 开发者自行测试常见攻击
无结构化流程，无文档 |
| Level 2: 工具辅助 | 使用自动化工具（Promptfoo/Garak）
集成到 CI/CD，有基本报告 |
| Level 3: 结构化 | 定期（季度）进行结构化红队测试
基于 MITRE ATLAS 的威胁模型
自动化 + 手动测试结合 |
| Level 4: 专业化 | 专业红队（内部或外包）
持续性红队（不是一次性的）
涵盖新攻击技术的研究跟踪 |
| Level 5: 生态化 | Bug Bounty 计划引入外部安全研究者
与行业安全社区协作
参与标准制定（OWASP、NIST） |
