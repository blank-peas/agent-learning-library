...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`vertical/customer-service.md`。
...
## 客服 Agent
  ...
  2024 年 Klarna 宣布 AI 客服替代 700 个全职人工、处理 2/3 的客服请求，让市场认识到客服可能是 LLM 应用第一个跑出 ROI 的垂直场景。客服 Agent 不是"会聊天的 ChatGPT"——它是分流路由、知识检索、订单查询、退款执行、人工升级的多 Agent 协作系统。本文讲清楚它的核心架构、SLA 设计、失败模式、以及为什么 OpenAI Agents SDK 的 Handoff 模型几乎就是为这个场景设计的。

  
    适合阶段：进阶 / 落地
    核心链路：Classify → Retrieve → Act → Escalate
    面试重点：分流策略 + Handoff vs Tool + 人工升级 SLA
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    为什么客服是 LLM 应用第一个跑出 ROI 的赛道？什么场景特性让它适合？
    考产品判断——任务边界清晰、有现成知识库、用户期望容忍度高（已习惯传统客服延迟）、ROI 可量化（替代人力 = 直接节省成本）。
  

  
    客服 Agent 的标准架构是分流 → 处理 → 升级，为什么不是单 Agent + 大 prompt？
    考多 Agent 必要性——客服任务的子类型差异大（订单 / 技术 / 投诉），单 Agent prompt 会膨胀且各类问题相互干扰，分流是健康多 Agent 的代表场景。
  

  
    客服 Agent 的"人工升级"什么时候触发？怎么交接才不让用户重复一遍问题？
    考 HITL 设计——升级触发条件（失败次数、敏感关键词、低置信度）+ 上下文摘要传递给人工坐席。
  

  
    客服 Agent 怎么避免"乱承诺"？比如答应退款但实际无权操作。
    考权限隔离——Agent 能"查"和"建议"，但"执行"动作（退款、改地址）走严格的工具权限，避免幻觉造成业务事故。
  

  
    客服 Agent 的成功率怎么评估？是看 CSAT 还是 deflection rate？
    考评估指标——deflection rate（不需要人工的比例）+ CSAT（用户满意度）+ FCR（一次解决率）+ AHT（处理时长），任何单一指标都误导。
  

  
    Klarna 说 AI 客服处理 2/3 请求，剩下 1/3 为什么不行？
    考边界认知——剩下的是高情感（投诉抱怨）、高复杂度（跨系统问题）、高风险（涉及金钱争议）场景，模型能力 + 法规 + 风险都还不允许全自动。
  

  
    多语言客服 Agent 怎么做？翻译 + 调用同一 Agent 还是 per-语言 Agent？
    考多语言架构——主流是"模型直接处理多语言 + per-语言知识库 + per-语言 prompt"，避免翻译损耗。
  

  
    客服 Agent 在生产里最常见的失败模式是什么？怎么治？
    考生产经验——幻觉政策、循环重试、用户挫败感、个性化失败、合规漏洞。
