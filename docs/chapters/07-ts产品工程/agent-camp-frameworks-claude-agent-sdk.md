...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`frameworks/claude-agent-sdk.md`。
...
## Claude Agent SDK 深度剖析
  ...
  Claude Agent SDK 不是又一个"调 API 的 wrapper"——它是 Anthropic 把 Claude Code 内核单独剥离出来的 SDK，自带文件系统工具、Bash 沙箱、Hooks 系统、permission flow 和 subagent 编排。如果你想做一个像 Claude Code 那样能自主编程的 Agent，这是离 Claude Code 最近的捷径。

  
    适合阶段：进阶 / 生产
    核心链路：Query → Tools → Hooks → Subagents → Permission
    面试重点：和 Anthropic SDK 边界 + Hooks 机制 + 内置工具集
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    Claude Agent SDK 和 Anthropic 的官方 SDK（anthropic-sdk-python）有什么本质区别？
    考层次理解——Anthropic SDK 是 API 客户端（messages.create），Agent SDK 是 Agent 运行时（含工具、循环、Hooks、subagents）。
  

  
    Agent SDK 内置的工具集（Bash/Read/Edit/Glob/Grep/WebFetch）为什么是这几个？少了/多了会怎样？
    考工具最小集的设计直觉——这是"读 + 写 + 执行 + 检索"的最小完备集，能让 Agent 完成绝大部分编程任务。
  

  
    Hooks 系统在哪些时机触发？和拦截器/中间件是一回事吗？
    考事件模型——PreToolUse / PostToolUse / Stop 等多个时机，可以中断、修改、增强 Agent 行为。
  

  
    subagent 和主 Agent 之间怎么传递信息？为什么不直接共享 context？
    考多 Agent 隔离设计——subagent 有独立 context（防污染主对话），通过 prompt + 返回结果通信。
  

  
    Permission flow 是怎么工作的？它和 OAuth 的 scope 有什么类比关系？
    考权限模型——四种 mode（acceptEdits/bypassPermissions/default/plan），每种代表不同的信任级别。
  

  
    为什么 Anthropic 要单独做这个 SDK，而不是让大家用 LangGraph 或者自己写循环？
    考产品战略——SDK 把 Claude Code 的"工程最佳实践"工业化、标准化，降低复刻成本。
  

  
    SDK 同时支持 TypeScript 和 Python，两套实现有差异吗？怎么保持一致？
    考工程实践——核心循环和工具语义对齐，但生态集成（如 MCP server）有差别。
