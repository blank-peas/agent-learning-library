...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`frameworks/openclaw.md`。
...
## OpenClaw 深度剖析
  ...
  OpenClaw 不是又一个"调 API 的 wrapper"——它是一个跑在你自己机器上的 AI 助手网关，把 WhatsApp、Telegram、Slack、iMessage 等 20+ 渠道统一接入同一个 Agent，所有数据留在本地。GitHub 37 万 star，2025 年个人 AI 助手赛道的事实标准。

  
    适合阶段：进阶 / 生产
    核心链路：Channel → Gateway → Agent → Tool/Skill
    面试重点：Gateway 架构 + Session 隔离 + Skill 自生成
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    OpenClaw 的 Gateway 和传统 API Gateway（如 Kong/Nginx）有什么本质区别？
    考架构理解——能不能讲清 Gateway 不只是流量转发，而是 Session 状态机 + 渠道路由 + 工具调度的控制平面。
  

  
    OpenClaw 怎么做到一个 Agent 同时服务 WhatsApp、Telegram、Slack？消息格式不一样怎么办？
    考多渠道抽象设计——Channel Adapter 层把平台差异抹平成统一的 Message 模型。
  

  
    多个用户同时给 Agent 发消息，Session 隔离怎么做的？会不会串上下文？
    考并发安全意识——per-sender session isolation，每个 sender 独立的对话状态。
  

  
    OpenClaw 的 Skill 和 MCP Server 有什么关系？能互相替代吗？
    考工具体系的分层认知——Skill 是更高层的"能力包"（含 prompt + tool + 文件），MCP 是底层的工具协议。
  

  
    Agent 自己写 Skill 给自己用，怎么防止它写出恶意代码？
    考安全边界设计——沙箱隔离（Docker/SSH/OpenShell）+ 非 main session 默认受限 + DM pairing code。
  

  
    "本地优先"在工程上意味着什么？和 SaaS 产品比有哪些 trade-off？
    考架构决策的工程直觉——隐私 vs 可用性、自运维 vs 托管、离线能力 vs 同步复杂度。
  

  
    OpenClaw 拿到 37 万 star 的核心原因是什么？技术壁垒在哪？
    考产品 + 技术结合的判断力——不是模型能力（它用的是 Claude/GPT），而是渠道整合 + 本地数据主权 + Skill 生态的网络效应。
