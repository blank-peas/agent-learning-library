# OpenAI Agents SDK / Swarm 深度剖析

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`frameworks/openai-agents-sdk.md`。
...
## OpenAI Agents SDK / Swarm 深度剖析
  ...
  2024 年 10 月 OpenAI 发布 Swarm（实验性多 Agent 框架），2025 年 3 月升级为正式产品 Agents SDK。核心理念是"最小原语，最大灵活"——只用 Agent、Handoff、Guardrail 三个抽象，就能搭出复杂的多 Agent 系统。这是 OpenAI 对"什么是 Agent 框架"的官方答卷。

  
    适合阶段：进阶 / 生产
    核心链路：Agent → Handoff → Guardrail → Tracing
    面试重点：Handoff vs Tool + Guardrails + Computer Use
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    Swarm 和 Agents SDK 是什么关系？为什么 OpenAI 要做两次？
    考产品演进——Swarm 是实验性 reference impl（教学用），Agents SDK 是 production-ready 升级版（带 tracing、guardrails、computer use）。
  

  
    Handoff 和普通的 Tool Call 有什么区别？为什么要单独设计一个原语？
    考多 Agent 设计——Tool 是"调用一个函数"，Handoff 是"把控制权转给另一个 Agent"，语义和上下文传递机制都不同。
  

  
    Guardrail 和 Hook 是一回事吗？什么时候触发？
    考守护机制——Input Guardrail 在 Agent 处理输入前跑，Output Guardrail 在 Agent 输出后跑，用并发模型不阻塞主链路。
  

  
    Agents SDK 的 Tracing 自动追踪是怎么实现的？性能开销大吗？
    考可观测设计——内置 OpenTelemetry 风格的 trace，记录每次 LLM 调用、tool call、handoff，可以导到第三方平台。
  

  
    Computer Use 工具是什么？它和普通的浏览器自动化（Playwright）有什么不同？
    考新一代 Agent 工具——LLM 直接看截图操作鼠标键盘，不依赖 DOM 解析或 selector，对动态页面更鲁棒。
  

  
    Agents SDK 和 LangGraph 比，最大的差距是什么？
    考框架定位——Agents SDK 走"轻量原语"路线（Agent/Handoff 几个核心概念），LangGraph 走"显式状态机"路线（节点+边+state+reducer）。
  

  
    OpenAI Assistants API（β 阶段那个）和 Agents SDK 是什么关系？
    考 API 历史——Assistants API 是服务端 Agent（OpenAI 帮你管状态），Agents SDK 是客户端框架（你的代码里跑循环）。
