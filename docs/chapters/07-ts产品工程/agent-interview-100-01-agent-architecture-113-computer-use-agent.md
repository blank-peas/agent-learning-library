...
> **资料来源**：[Agent Interview 100](https://github.com/BigKunLun/Agent-Interview-100) · 原文件：`01-agent-architecture/113-computer-use-agent.md`。
...
```mermaid
flowchart TD
    T["用户任务"] --> P["感知<br/>截图 / DOM"]
    P --> PL["规划<br/>ReAct 决策下一步"]
    PL --> EX["执行<br/>click / type / scroll"]
    EX --> V{"校验:子目标达成?"}
    V -- "未达成" --> F["修复 / 回溯"]
    F -->|"重新感知"| P
    V -- "达成" --> R["任务完成 / 主动放弃"]
    ...
    class T,P,PL,EX,R proc
    class V decision
    class F err
```
*感知→规划→执行→校验闭环：每步校验子目标，未达成则修复/回溯再感知——这是 Computer Use Agent 区别于盲操作脚本的关键。*
