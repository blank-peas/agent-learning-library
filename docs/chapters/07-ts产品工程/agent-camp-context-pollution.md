...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`context/pollution.md`。
...
## 上下文污染与清理
  ...
  错误累积、Token 中毒、上下文漂移——长程 Agent 跑着跑着就"疯了"的根本原因，以及怎么主动清理。
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    一个 Agent 跑了 30 步后输出和最初任务完全无关，根因是什么？
    考能不能识别 context drift 这种长程退化模式，区分它和单步错误。
  

  
    Token 中毒和 prompt injection 是同一回事吗？区别在哪？
    考概念辨析——一个是恶意的安全问题，一个是更广的数据质量问题。
  

  
    怎么自动检测上下文被污染了？有哪些可量化指标？
    考有没有工程化思维，能不能给出具体监控信号而不是"感觉不对"。
  

  
    Claude Code 的 /clear 命令背后什么逻辑？为什么不能直接靠长上下文撑下去？
    考对生产 Agent 上下文管理的理解，不是单纯"上下文越长越好"。
  

  
    Reflexion / 自我纠错机制能清理上下文污染吗？
    考对自我纠错的边界认知——它能纠错但不能根治污染。
  

  
    压缩上下文（summarization）会消除污染还是加剧？
    考压缩这把双刃剑的理解。
  

  
    多 Agent 协作里，怎么避免一个 Agent 的污染传染给下游？
    考分布式系统的污染隔离思维。
