# Agent 高可用与容灾面试深挖：别把“没做双活”答成短板

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`engineering/reliability-interview.md`。
...
## Agent 高可用与容灾面试深挖：别把“没做双活”答成短板
  ...
  面试官问“上游挂了怎么办”，真正想听的是你有没有按业务等级设计恢复目标，而不是有没有盲目堆一套昂贵双活。

  
    适合阶段：Agent 工程师 / 技术负责人面
    核心能力：RTO/RPO · 依赖分级 · 降级模式 · 演练复盘
    脱敏原则：只保留容灾方法，不保留真实业务细节
...
这组题看似在问运维，实际在考你是不是能把 AI 应用当成生产系统，而不是一个“能跑就行”的模型调用链。
    ...
    如果主模型供应商大面积 5xx 或延迟飙升，你的 Agent 怎么办？
    考 provider 级容灾、模型兼容、降级口径和用户体验。
  

  
    为什么没有直接做跨云双活？怎么判断值不值得？
    考 RTO/RPO、成本、复杂度和业务等级，而不是“有钱就双活”。
  

  
    Agent 的 RTO 和 RPO 怎么定义？是不是全系统一个值？
    考你能不能按用户流程分级，而不是套传统系统模板。
  

  
    备用模型切上去后，工具调用 schema 不兼容怎么办？
    考 fallback 不是换 endpoint，而是完整适配工程。
  

  
    向量库、工具 API、LLM、数据库分别挂了，降级策略一样吗？
    考依赖分级和故障矩阵，能不能避免“一刀切不可用”。
  

  
    容灾方案怎么验证？你怎么证明备用链路真的能接流量？
    考演练、混沌测试、runbook 和故障注入。
  

  
    出现部分失败时，是返回错误、排队、转人工，还是给简版结果？
    考产品体验和工程风险边界。
  

  
    上线后应该看哪些可靠性指标？只看可用率够不够？
    考 degraded rate、failover time、stale answer、队列积压和人工接管。
