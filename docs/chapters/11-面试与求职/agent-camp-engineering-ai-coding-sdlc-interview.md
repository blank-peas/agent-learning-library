# AI Coding SDLC 面试深挖：别只说“代码大多让 AI 写”

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`engineering/ai-coding-sdlc-interview.md`。
...
## AI Coding SDLC 面试深挖：别只说“代码大多让 AI 写”
  ...
  面试官真正想听的是：你怎么让 AI 生成的代码可理解、可验证、可评审、可回滚。

  
    适合阶段：Agent 工程师 / 技术负责人面
    核心能力：PRD · Tech Design · Task · TDD · Review Gate
    脱敏原则：只保留方法论，不保留真实项目细节
...
这些问题听起来在问“你会不会用 AI 写代码”，实际是在问你有没有软件工程控制力。
    ...
    你说 AI 写了大量代码，那你负责什么？
    考责任边界：需求理解、架构设计、风险判断、review 和验收不能外包给模型。
  

  
    AI Coding 怎么嵌入 PRD、技术方案、任务拆解和开发流程？
    考 SDLC 视角，不接受“我打开工具让它写”。
  

  
    PRD 很模糊时，AI 应该直接开写还是先澄清？
    考需求 gate：可验收、可拆分、无无关愿望项。
  

  
    为什么 TDD 在 AI Coding 里反而更重要？
    考你是否理解代码有天然 verifier，测试是 reward 也是安全带。
  

  
    怎么防止 Agent 乱改无关代码、越权改核心模块？
    考权限、diff 粒度、高风险文件保护和 owner approval。
  

  
    AI 生成代码怎么做 review？人审和 AI review 怎么分工？
    考 maker/checker 分离、审查清单和责任归属。
  

  
    怎么评估 AI Coding 是否真的提高研发效率，而不是制造维护债？
    考效率、质量、采纳率三类指标。
  

  
    线上问题排查能不能也交给 AI？边界在哪里？
    考日志权限、复现脚本、只读数据、生产库边界和审计。
