# Agent 线上回复监测面试深挖：接口成功，不代表回答正常

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`engineering/online-response-monitoring-interview.md`。
...
## Agent 线上回复监测面试深挖：接口成功，不代表回答正常
  ...
  面试官问“Agent 上线后怎么确保回复正常”，真正想听的是你有没有把非确定性输出变成可监测、可告警、可回放的生产信号。

  
    适合阶段：Agent 工程师 / 平台架构面
    核心能力：response health · online eval · implicit feedback · semantic SLO
...
这组题能区分“会接模型 API”和“能守线上系统”的候选人。
    ...
    Agent 接口返回 200，但回答是错的，线上监控怎么发现？
    考你是否知道 semantic failure，不只看 HTTP status。
  

  
    传统接口有成功/失败，Agent 回复状态不固定，怎么定义正常？
    考 response health schema、任务完成度和质量维度拆分。
  

  
    哪些信号适合全量监测，哪些只能采样或异步评估？
    考规则、任务裁判、LLM judge、人工抽检的成本与延迟边界。
  

  
    用户没点踩，你怎么发现 Agent 回复有问题？
    考重复追问、纠错、转人工、fallback、工具矛盾、低置信度等隐式信号。
  

  
    线上告警怎么设？什么情况 P0，什么情况只进 badcase 队列？
    考安全、任务失败率、语义异常率、judge 漂移和告警疲劳。
  

  
    怎么监测“模型今天变差了”或 prompt 发布后质量退化？
    考 canary、shadow、slice-level metrics、case-level diff 和版本化 trace。
  

  
    线上每条都跑 LLM judge 会不会成本和延迟爆炸？
    考主链路规则、异步裁判、风险分级采样和夜间批处理。
  

  
    怎么把线上监测结果变成修复和回归？
    考从 alert 到 trace，到归因，到 dataset slice，再到发布门禁。
