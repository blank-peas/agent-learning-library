# Agent 首 Token 与 P95 延迟面试深挖：别把 8 秒答成“模型有点慢”

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`engineering/latency-interview.md`。
...
## Agent 首 Token 与 P95 延迟面试深挖：别把 8 秒答成“模型有点慢”
  ...
  面试官问“首 token 8 秒花在哪”，真正想听的是你会不会拆关键路径、看分位数、按收益排序优化。

  
    适合阶段：Agent 工程师 / 后端架构面
    核心能力：TTFT · critical path · P95 · prompt cache · prefetch
    脱敏原则：只保留性能方法，不保留真实业务或内部数量
...
这些题表面是性能优化，实际考的是你有没有生产系统的定位习惯。
    ...
    一个 Agent 首 token P95 8 秒，你怎么定位瓶颈？
    考 trace 分段、关键路径和分位数，而不是笼统说“模型慢”。
  

  
    TTFT、总耗时、输出速度、工具耗时分别看什么？
    考你是否能区分用户感知延迟和后台执行延迟。
  

  
    平均 4 秒、P95 8 秒，优化策略和平均 8 秒一样吗？
    考尾延迟治理：长尾工具、缓存 miss、队列等待和偶发重试。
  

  
    为什么 prompt cache / prefix cache 会影响首 token？
    考 prefill、KV cache、稳定前缀和压缩副作用。
  

  
    哪些步骤能并行，哪些不能？怎么证明并行真的省时？
    考 critical path，而不是“把所有 await 都 gather”。
  

  
    流式输出是不是一定能降低延迟？
    考 perceived latency 与真实端到端耗时的区别，以及安全 buffer 的代价。
  

  
    什么时候应该用小模型、语义缓存、模板，而不是继续优化大模型？
    考“不要默认用 LLM”的工程判断。
  

  
    性能优化上线后，你看哪些指标防止质量回退？
    考性能、质量、安全和成本的联动治理。
