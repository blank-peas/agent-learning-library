# Agent Runtime 面试深挖：从业务 Agent 讲到平台引擎

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`engineering/agent-runtime-interview.md`。
...
## Agent Runtime 面试深挖：从业务 Agent 讲到平台引擎
  ...
  面试官追问的不是“你接了几个工具”，而是“谁控制循环、谁校验动作、谁为结果负责”。

  
    适合阶段：Agent 工程师 / 平台架构面
    核心能力：架构命名 · 机制边界 · 追问应对
    脱敏原则：只保留技术问题，不保留公司、项目、指标细节
...
这些题通常不会按顺序出现。真实面试里，面试官会先让你讲一个项目，然后沿着你回答里最虚的一处往下追。
    ...
    你能不能先用一句话概括这个 Agent 的整体架构？
    考你能不能先命名架构，而不是从意图识别、工具调用这些实现细节开始散讲。
  

  
    你说这是受限 Agent，受限在哪里？模型到底有哪些自主权？
    考你能否把局部自主和系统控制权分开。
  

  
    工具执行由谁决定？loop 是模型主控，还是 orchestrator 主控？
    考 Agent loop 的控制边界，尤其是候选动作和最终执行的区别。
  

  
    Reflection 还是模型自己生成的，为什么这就叫 harness 控制？
    考你是否知道反思只是候选状态更新，采纳必须由 schema、verifier、state machine 或人控。
  

  
    如果现在重做，你会从业务 Agent 升级到什么形态？
    考架构演进能力：tool registry、memory service、guardrail、eval/trace 是否能沉成平台能力。
  

  
    高风险场景里，流式输出已经展示给用户后才发现违规怎么办？
    考输出侧 guardrail，尤其是服务端 buffer、abort/retract、审计和降级策略。
  

  
    长期记忆很多时怎么按需使用？什么时候允许写入？
    考 memory governance，而不是只会说“存数据库”或“查向量库”。
  

  
    你为什么选这个框架、RAG 方案或观测平台？有没有对比和退出条件？
    考选型判断：阶段性选择、评估指标、替换条件，而不是“当时就用了”。
