# Agent 线上质量治理面试深挖：别只答点赞和人工 review

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`engineering/agent-quality-interview.md`。
...
## Agent 线上质量治理面试深挖：别只答点赞和人工 review
  ...
  面试官问线上质量，不是在问你有没有用户反馈按钮，而是在问你能不能把 Agent 当成可治理的生产系统。

  
    适合阶段：Agent 工程师 / 技术负责人面
    核心能力：Trace · Task Completion · Judge · Badcase Regression
    脱敏原则：只保留质量治理方法，不保留业务数据
...
这组问题最容易把候选人从“做过 AI 项目”打回“只是接过模型 API”。它考的不是名词，而是线上系统治理能力。
    ...
    上线后怎么评估 Agent 效果？除了点赞 / dislike / 人工 review 呢？
    考你是否知道用户反馈是弱信号，能不能讲出线上 trace + 自动裁判 + badcase 闭环。
  

  
    一个业务 Agent 的质量应该拆成哪些维度？
    考你是否只看最终文本，还是能拆任务完成、工具正确、内容可信、安全合规、体验成本。
  

  
    任务完成度怎么定义？怎么自动判断？
    考你能不能把“回答好不好”转成可由 trace 判断的 completion checklist。
  

  
    自动裁判是不是就是 LLM Judge？哪些地方应该用规则？
    考你是否知道规则裁判、任务裁判、LLM Judge、人工抽检各自边界。
  

  
    用户没点差评，你怎么发现 badcase？
    考隐式信号：重复追问、转人工、fallback、工具失败、低置信度、高风险意图。
  

  
    一个 badcase 修完后，怎么保证老 case 不回归？
    考 case 级 diff、pass→fail 回归、分层 pass rate，而不是只看总分。
  

  
    线上每条都跑 LLM Judge，成本会不会爆？
    考采样策略：规则全量、Judge 异步抽检、高风险必审。
  

  
    安全类问题怎么评估？能不能和普通回答质量放一个分数里？
    考一票否决、安全漏拦/误拦、人工复核和独立 guardrail。
