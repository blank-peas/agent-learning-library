# Agent 流式输出安全面试深挖：已经吐出去的内容，不能靠前端改写补救

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`engineering/streaming-guardrail-interview.md`。
...
## Agent 流式输出安全面试深挖：已经吐出去的内容，不能靠前端改写补救
  ...
  面试官追问“流式输出中途命中风险，前面已经展示的内容怎么办”，问的不是敏感词，而是你有没有把输出链路当成生产安全边界。

  
    适合阶段：Agent 工程师 / 平台架构面
    核心能力：buffer 审查 · guardrail · latency trade-off · audit
    脱敏原则：只保留技术方法，不保留真实业务细节
...
这组问题看上去像安全题，实际在考你是否理解“用户看到内容”也是一个不可逆副作用。
    ...
    输入侧已经做了安全分类，为什么输出侧还要审查？
    考你是否知道模型、RAG、工具结果都会引入新的风险，输入安全不等于输出安全。
  

  
    流式输出中途命中风险，前面已经展示给用户的内容怎么办？
    考你是否把展示当成副作用，而不是事后把前端 DOM 改一下。
  

  
    句子级 buffer 审查会增加首 token 延迟，怎么取舍？
    考你会不会按风险等级选择同步、异步、模板化和非流式模式。
  

  
    关键词、规则、分类器、LLM judge，输出审查应该怎么组合？
    考你是否能把快路径、准路径和人工路径分层，而不是只说“加个模型”。
  

  
    guardrail 服务挂了，Agent 是否还能继续流式输出？
    考安全依赖的降级策略：fail-open、fail-closed 和按风险分级。
  

  
    为什么有些场景不应该做 token 级流式？
    考你是否敢为了安全牺牲体验，而不是把 streaming 当默认功能。
  

  
    输出审查怎么评估？只看拦截率够不够？
    考 recall、false positive、延迟、撤回率、人工复核一致率这些指标。
  

  
    如果前端已经展示过风险内容，日志和产品语义应该怎么设计？
    考审计、retract event、用户提示、case 回流和事故复盘能力。
