...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`rag/evaluation.md`。
...
## RAG 评估 Evaluation
  ...
  把 retrieval、grounding、answer quality 分开测，才能知道 RAG 到底错在哪里。

  
    适合阶段：上线前与索引变更后
    核心机制：Dataset / Metrics / Trace / Regression
    面试重点：retrieval vs generation 归因
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    retrieval metrics 和 generation metrics 分别测什么？
    考 recall@k、MRR、NDCG、faithfulness、answer correctness 的边界。
  

  
    context precision 和 context recall 怎么理解？它们和 IR 里的 precision/recall 有什么差别？
    考 RAGAS / DeepEval 这类指标背后的数据假设。
  

  
    LLM-as-judge 可靠吗？怎么减少 judge 自己带来的噪声？
    考 rubric、校准集、人工抽检、结构化输出。
  

  
    评估集怎么构造？只靠 synthetic Q&A 会出什么问题？
    考日志采样、困难样本、no-answer、权限样本。
  

  
    RAGAS、TruLens、DeepEval、LangSmith 这几类工具怎么选？
    考指标框架、trace、实验管理的分工。
  

  
    知识库重建、embedding 换模型、chunking 调参后怎么防回归？
    考 CI、版本化 eval set、分 slice 看指标。
