# Agent 离线评测数据集：把线上 badcase 变成回归测试

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`engineering/offline-eval-datasets.md`。
...
## Agent 离线评测数据集：把线上 badcase 变成回归测试
  ...
  Agent 评测最难的不是跑一个分数，而是把真实业务里的用户、工具、状态和成功标准冻结成一批可重复执行的任务。

  
    适合阶段：生产 Agent / 上线前回归
    核心：trace 采样 + 状态判分 + grader 校准
    面试重点：数据构建 + 评测可信度 + badcase 闭环
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    Agent 离线评测集应该从哪里来，为什么不能全靠人工编题？
    考真实分布意识：生产 trace、失败 case、人工边界样本、合成样本各自怎么占比。
  

  
    一个 eval case 里必须存哪些字段？只存用户问题够不够？
    考能不能把输入、环境 fixture、期望 outcome、判分规则、metadata 分开。
  

  
    客服退款 Agent 怎么做离线评测，判分看回复文本还是数据库状态？
    考 outcome-based grading：Agent 说“已退款”不算，订单状态真的变了才算。
  

  
    怎么证明这个评测集能发现真实回归，而不是只给团队制造幻觉？
    考评测有效性：覆盖率、A/A 噪声、grader-human agreement、已知回归注入。
  

  
    LLM judge 和规则判分怎么组合？哪些地方必须用 deterministic grader？
    考分层判分能力：状态、工具、格式用规则，语义质量才交给 judge。
  

  
    评测分数从 86% 升到 88%，该不该上线？
    考统计意识：置信区间、重复 trial、slice 分析和业务风险比单点分数重要。
  

  
    评测集跑到 98% 通过率，是好消息还是坏消息？
    考 saturation：能守回归但不再区分进步，要补更难的新任务。
  

  
    怎么防止团队过拟合自己的离线评测集？
    考数据版本、holdout、私有集、在线反馈回流和人工抽查。
