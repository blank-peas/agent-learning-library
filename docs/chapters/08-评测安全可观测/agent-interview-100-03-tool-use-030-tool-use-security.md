...
> **资料来源**：[Agent Interview 100](https://github.com/BigKunLun/Agent-Interview-100) · 原文件：`03-tool-use/030-tool-use-security.md`。
...
flowchart LR
    ...
    T --> X["外部系统<br/>数据库 / API"]
    X -- "工具返回值" --> L
    I1["输入注入<br/>用户藏恶意指令"] -.-> L
    I2["间接注入<br/>返回值藏恶意指令"] -.-> L
    I3["Tool Poisoning<br/>恶意 MCP Server"] -.-> L
    ...
    class I1,I2,I3 err
```
*三个注入入口——用户输入、工具返回值（间接注入）、恶意 MCP Server——都汇向 LLM，再流向工具执行。*
...
```mermaid
flowchart TD
    A["第 1 层 · 输入防护<br/>输入清洗 · 长度限制 · 意图分类"] --> B["第 2 层 · LLM 层防护<br/>安全指令 · 指令/数据分离 · CoT"]
    B --> C["第 3 层 · 工具执行防护<br/>Schema 验证 · 注入检测 · Gateway 权限"]
    C --> D["第 4 层 · 运行时防护<br/>限流 · 异常检测 · 审计 · HITL"]
    D --> E["第 5 层 · 输出防护<br/>返回值清洗 · PII 过滤 · 一致性检查"]
    classDef proc fill:#e3f2fd,stroke:#1565c0,color:#0d47a1
    class A,B,C,D,E proc
```
*五层纵深防御——输入、LLM、工具执行、运行时、输出，每层都假设上一层已被绕过。*
