# MCP Server 生产化：从能连到可控可部署

> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`tools/mcp-production.md`。
...
## MCP Server 生产化：从能连到可控可部署
  ...
  MCP demo 跑通只证明协议通了，生产环境还要管住身份、权限、版本、审计和失败。

  
    适合阶段：工具生态 / 生产部署
    核心机制：Transport + Auth + Policy + Observability
    面试重点：MCP 原型和生产系统的差距
...
### 面试官想考什么
    ...
    一个 MCP Server 从本地 demo 到生产服务，中间缺哪些层？
    考你是否理解工具协议之外的工程边界。
  

  
    stdio 和 Streamable HTTP 怎么选？什么时候不该上远程 MCP？
    考部署形态、认证、延迟和数据边界。
  

  
    MCP Server 的认证应该放在哪里？token 能不能放 tool description？
    考 secret 管理和提示词泄漏风险。
  

  
    tool schema 改了，已经连接的 Agent 怎么办？
    考版本管理、兼容性和会话隔离。
  

  
    为什么 Resource 和 Prompt 在生产 MCP 里很重要？
    考你有没有超越"所有能力都写成 tool"的阶段。
  

  
    怎样防止恶意 MCP Server 或 tool poisoning？
    考供应链、安全审核和权限最小化。
  

  
    MCP 工具调用应该怎么做 tracing？
    考线上排障：谁调的、调了什么、返回了什么、花了多少钱。
