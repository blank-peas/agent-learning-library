...
> **资料来源**：[Agent Interview 100](https://github.com/BigKunLun/Agent-Interview-100) · 原文件：`09-safety-and-alignment/079-guardrails-basics.md`。
...
```mermaid
flowchart TD
    A["用户请求"] --> IN["输入护栏<br/>PII 脱敏 · 注入检测<br/>话题合规 · 长度格式"]
    IN --> P["Prompt 构造"]
    P --> L["LLM"]
    L --> OUT["输出护栏<br/>敏感过滤 · 幻觉检测<br/>毒性检测 · 结构验证"]
    OUT --> R["返回用户"]
    L -- "Agent 特有" --> TG["工具调用护栏<br/>权限检查 · 参数验证<br/>危险操作审批"]
    TG --> T["工具执行"]
    classDef decision fill:#fffde7,stroke:#f9a825,color:#795548
    classDef agent fill:#f3e5f5,stroke:#6a1b9a,color:#4a148c
    classDef proc fill:#e3f2fd,stroke:#1565c0,color:#0d47a1
    class IN,OUT,TG decision
    class L agent
    class A,P,R,T proc
```
*护栏设三道卡：进 LLM 前、出 LLM 后、工具执行前——Agent 比聊天机器人多一道工具调用护栏。*
...
| 先规则后模型 | 规则型快速拦截明显问题，模型型处理需要语义理解的场景 |
| 失败安全 | 护栏出错时默认拒绝而非放行 |
