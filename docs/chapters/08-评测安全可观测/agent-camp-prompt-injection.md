...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`prompt/injection.md`。
...
## Prompt Injection 攻防
  ...
  LLM 应用的 #1 安全风险——直接注入、间接注入、越狱、数据泄漏、工具滥用，以及多层防御策略。
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    什么是 prompt injection？和 SQL injection 有什么本质区别？
    考概念辨析，能不能讲清"为什么 LLM 注入更难防"。
  

  
    直接注入和间接注入区别是什么？哪个更危险？
    考你知不知道 2023 后真正危险的攻击类型。
  

  
    越狱（jailbreak）和注入是同一回事吗？
    考概念区分。
  

  
    OWASP LLM Top 10 里 prompt injection 排第几？还有哪些重要风险？
    考安全意识体系。
  

  
    为什么"用更强的 system prompt 警告模型"防不住注入？
    考对 LLM 本质的理解。
  

  
    生产 Agent 防注入的多层防御应该怎么设计？
    考工程实战，深度防御思维。
