...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`engineering/observability.md`。
...
## Agent 可观测性
  ...
  Agent 是 N 步 LLM 调用 + M 次工具调用串起来的非确定性黑盒——出了问题没有 trace 就只能凭运气复现。这篇讲怎么用 trace/span 模型把执行过程结构化记下来、用哪家工具记、记什么不记什么、出问题怎么从日志反查到根因。

  
    适合阶段：生产 Agent 必备
    核心：trace/span + 工具选型 + debug 工作流
    面试重点：和传统 APM 的差异 + PII 处理
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    Agent observability 和传统服务可观测性差别在哪？
    考能不能讲出非确定性输出、多步上下文、token 计费、prompt/response 全文留存这四类特殊性。
  

  
    trace 和 span 模型怎么映射到 Agent 的执行过程？
    考能不能把"一次用户请求 = 1 trace，每次 LLM/tool/retrieval 调用 = 1 span"这套层级讲清楚。
  

  
    LangSmith / Langfuse / Phoenix / Helicone 怎么选？
    考工具辨析能力——SaaS vs self-host、proxy vs SDK、生态绑定，能给出有理由的推荐。
  

  
    用户投诉 Agent 答错了，从 observability 数据怎么定位？
    考实战 debug 流程：从 user_id → session_id → trace → 重放 prompt 复现的完整链路。
  

  
    PII（手机号、身份证、银行卡）出现在 prompt 里，trace 怎么处理？
    考合规意识——脱敏在哪一层做、能不能丢、留多久。
  

  
    observability 应该记什么？什么不能记？
    考工程取舍——既不能 log 太少（debug 不了）也不能 log 太多（成本爆炸 + 合规风险）。
  

  
    怎么把 observability 数据反哺到评估？
    考闭环思维——生产 trace 怎么变成回归集、bad case 怎么进 dataset。
  

  
    OpenTelemetry GenAI semantic conventions 解决什么问题？
    考是否关注业界标准化——为什么需要跨工具的 schema，不绑死单家厂商。
