...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`engineering/security.md`。
...
## Agent 整体安全与纵深防御
  ...
  OWASP LLM Top 10 全景、护栏框架对比、PII 脱敏、红队测试与生产级 Agent 的五层纵深防御。

  
    适合阶段：生产化必读
    核心：威胁模型 + 工程化纵深防御
    面试重点：能不能讲清"为什么单点防御必败"
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    OWASP LLM Top 10 里你印象最深的三条是什么？为什么这三条？
    考你有没有完整的威胁模型，不是只盯 prompt injection。
  

  
    LLM06 Excessive Agency 是什么？它和 prompt injection 怎么区别？怎么防？
    考你对"权限放大"这种 Agent 特有风险的理解。
  

  
    PII 脱敏的"可逆"和"不可逆"各自适合什么场景？
    考你能不能从用户场景反推技术选型，不是死记两种方案。
  

  
    Guardrails AI / NeMo Guardrails / Llama Guard 三类护栏怎么选？
    考你看过这些库，知道 input validator、Colang DSL、安全分类模型的分工。
  

  
    Agent 红队测试和传统渗透测试有什么不同？
    考你知不知道 garak、PyRIT、AdvBench，懂"概率系统"和"确定性系统"测试差异。
  

  
    多租户 Agent 平台怎么防 cross-tenant 信息泄漏？
    考你对 SaaS Agent 数据隔离的真实工程经验。
  

  
    GDPR / EU AI Act 对 Agent 应用有哪些硬性约束？
    考你知不知道合规不是"加个 cookie banner"那么简单。
  

  
    一个新 Agent 产品上线前，你怎么设计完整安全检查清单？
    考你的端到端工程化思维。
