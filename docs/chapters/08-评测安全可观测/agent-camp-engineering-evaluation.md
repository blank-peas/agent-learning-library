...
> **资料来源**：[Agent Camp](https://github.com/yibo365/agent-camp) · 原文件：`engineering/evaluation.md`。
...
## Agent 评估体系
  ...
  主流 benchmark、自建评估集、评估方法论——为什么 Agent 评估比 LLM 评估难得多，以及怎么搭一个生产可用的评估流水线。

  
    适合阶段：生产 Agent 必读
    核心：benchmark + 自建集 + 在线反馈三层
    面试重点：评估难点 + 指标体系 + 排坑
...
读完这篇你要能正面回答下面这些题。每题后面括号里是面试官真正想看你答出什么。
    ...
    考能不能识别多轮交互、工具非确定、没有标准答案这些结构性差异。
  

  
    SWE-bench 的设计为什么这么有区分度？
    考对真实 benchmark 设计要素的理解——任务源、自动判分、防泄露。
  

  
    怎么从零给一个客服 Agent 建评估集？
    考能不能落地——case 采样、标注、维护、版本化。
  

  
    线下评估 90% 通过，上线后用户反馈差，怎么 debug？
    考评估集与真实分布偏离的诊断思路。
  

  
    评估指标该看 Pass@1 还是 Pass@K？
    考对 Agent 多次尝试的成本/收益辨析，不是机械套公式。
  

  
    GAIA 和 SWE-bench 各自考察什么能力？什么场景该选哪个？
    考主流 benchmark 的覆盖差异——通用助手 vs 编程修复。
  

  
    LLM-as-judge 评估 Agent 输出有什么偏差？怎么校正？
    考自动评估的边界，引出 judge 偏差专题。
  

  
    评估成本失控怎么办？一次跑完 SWE-bench Verified 多少钱？
    考工程现实——评估本身是高成本投入，要预算管理。
